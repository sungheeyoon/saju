...
그 정의를 지키는 까닭이고, `_Avoid_` 는 같은 것을 다르게 부르던 말이다. 용어는 한국어이고
코드는 영어라 둘을 잇는 표가 §9 에 있다 — **그 표의 식별자가 코드에 실제로 있는지는
...
거둔 상태. 계정은 그대로 있고 자기 것을 읽을 수도 있다 — 못 하게 되는 것은 새로 만들고
청하는 일이다. 운영자가 이 상태를 거는 일을 **제재**라 부르고, 그것은 **신고의 결론이지
...
**기본으로 켜져 있다** — 사주를 저장한 사람은 자동으로 후보 풀에 들고, 그 고지는 **가입
폼**이 읽힌다. 켠 시각과 **끈 시각**을 함께 드는 **사건**이지 설정값이 아니다. 끄면 내놓은
...
User 의 **selfPerson**. 대신 등록한 Person(엄마·친구)은 후보가 되지 않는다. 추천 목록은
**스냅샷**이다(`discovery_snapshot`) — 뽑는 자리와 읽는 자리가 갈려 있다.
_Avoid_: 상대, 매칭 대상
...
| `suspended` 하나 | 중지 · 제재 | `app_user.status` | 상태와 그 상태를 거는 일을 용어집이 두 말로 부른다. 값은 하나가 맞다 |
| 이름 없음 | DiscoveryProfile | TS | 표만 있고 도메인 타입이 없다 — 읽는 문이 칸을 그대로 든다 |
| `discovery_snapshot` | 후보 목록(스냅샷) | 표 | 「스냅샷」이 여덟 글자 스냅샷과 겹친다 — 용어집은 추천 목록 쪽에 이 말을 양보했다 |
