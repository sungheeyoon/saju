...
**엔진이 끝까지 판정하는 것은 억부 하나다** — 조후는 문턱 없는 조건만 판정한 **조건부 판정**이고
급함은 정하지 않는다. 통관은 사실만, 병약은 없다(ADR 0111).
...
있고 이 엔진에 그 자리가 없다. 무작위 3000건에서 어긋나는 명식이 43.2% 다. 「겹치는 글자가 있는가」
하나라 조후가 억부의 무거운 쪽을 함께 보태도 맞는다고 센다 — 그 자리를 가르는 것은 **억부·조후 관계**다.
_Avoid_: 「조후가 우선이다」(재지 않은 것을 말하는 것이다)

**억부 판정(구조)** — `eokbuJudgementOf` : 억부 오행 하나(`eokbuAssessmentOf`)를 1순위로 그대로 두고
대안 후보 · 후보가 앉은 자리(드러남 · 본기 · 숨음 · 없음, 일간 제외) · 불균형의 단계 · 가장 무거운 쪽 ·
시를 모를 때 열두 시 중 같은 답의 수를 함께 든 값. 세기는 단계뿐이고 신뢰도는 `low` 에서 시작한다 —
외부 억부 20건 중 1순위가 맞는 것이 10건이다. 대안(신약 · 재성 최다 → 보이는 인성)은 같은 스무 건으로
맞춘 규칙이라 1순위를 못 바꾼다(ADR 0111).
_Avoid_: 「용신」(억부 한 길의 답이다), 가중치 · 점수(단계로만 말한다)

**조후 판정(조건부)** — `johuJudgementOf` : 조후표 칸의 조건 중 문턱 없이 답하는 것(상 · 하반월 · 앞
글자의 실재 · 그 글자의 뿌리 · 꺼림)만 판정해 권하는 **천간**마다 살았는지를 든 값. 국 · 세력 문턱 ·
적히지 않은 조건은 판정하지 않고 그 종류를 남긴다. 급함은 `unresolved`, 신뢰도는 늘 `low` — 조후
외부 대조가 0건이다. 계절의 한난조습은 월지에서만 말하고 원국 전체의 것은 말하지 않는다.
_Avoid_: 「화가 필요하다」(丙을 오행으로 줄이는 말이다), 「조후가 급하다」(잰 적이 없다)

**억부·조후 관계** — `eokbuJohuRelationOf` : 두 판정이 겹치는가 · 부딪는가를 다섯 갈래(같은 쪽 ·
일부 · 충돌 · 견줄 수 없음 · 모자람)로 가른 값과 서열의 신호. 충돌은 조후 글자가 억부의 **가장 무거운
쪽**을 보탤 때만이고, 억부가 말하지 않은 오행은 견줄 수 없음이다. **여전히 어느 쪽이 우선인지는 말하지
않는다** — 같은 쪽이면 둘이 서로를 받친다(`reinforced`)고만 하고, 나머지는 `unresolved` 와 그 까닭이다.
무작위 1000건에서 같은 쪽 38 · 일부 390 · 충돌 373 · 견줄 수 없음 199 건이다.
_Avoid_: 「조후가 우선이다」 · 「억부가 우선이다」(둘 다 잰 적이 없다)

**필요 오행 프로필** — `needProfileOf` · `NeedProfile` : 한 사람이 무엇을 필요로 하는가 — 억부 1순위 ·
대안 · 조후가 권한 글자를 줄로, 억부의 가장 무거운 쪽을 넘치면 불리한 것으로, 관계와 못 정한 것을
함께 든 값. **한 사람만 본다** — 두 사람 사이의 보완(A→B · B→A)은 이것을 사람마다 한 번씩 읽을 다음
일이다. **궁합 점수가 아니고 예측 궁합 점수와도 다르다**(`COMPAT_POLICY.scoring: 'not-scored'`).
...
**풀이 때 다시 잰** **예측 궁합 점수**이고 눈금은 사이가 고른다(`previewScoreOf`, ADR 0113). 풀이는 기준점 · 판 ·
그때의 사이를 함께 저장한다(`reading.score_baseline` · `score_version` · `score_relation`). 모델은 여기서 여덟 항목으로 ±15 안에서 움직인다
...
**연인용**(후보 카드 · 성립한 인연 · 연인 · 배우자)은 일주 · 일지 관계 40% · 필요한 기운 보완 40% · 오행 균형 20%,
**일반**(가족 · 친구 · 동료 · 모름)은 필요한 기운 보완 60% · 오행 균형 40% 다. 진입점은 하나이고 정책을 받는다 — 사이 →
정책은 `scorePolicyOf` 한 자리가 정한다. 무게는 가설이다. 옛 판 `discovery-v1`(균형 70 · 개수 보완 30)은 그 판으로 만든
풀이를 다시 여는 자리에만 남는다(`legacyPreviewScoreOf`). 화면에 판본 이름을 쓰지 않는다. **이름은 제품을
