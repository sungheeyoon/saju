...
_Avoid_: 중지(2026-09-23 까지의 이름 — DB 가 던지는 문장에 아직 남았다, G-49), 삭제, 차단, 잠김
...
처분은 운영자가 하고(runbook 「탈퇴 신청의 처리」), 무엇을 지우고 익명화하고 남길지는 공개 출시
전에 정한다. 이미 성립한 **Match** 처럼 두 사람의 것인 기록은 한쪽이 지울 수 없다.
_Avoid_: 계정 삭제 요청 · 삭제 요청(2026-09-23 까지의 이름), 삭제(그 자리에서 사라진다고 읽힌다)

**탈퇴** — 코드에 없다 : 탈퇴 대기의 유예기간이 끝나 운영자가 처분을 마친 상태. `app_user` 에
값이 따로 없다 — **행이 사라지는 것이 곧 이 상태**다. 남은 사람의 화면에서 그 사람의 이름이
서던 자리(대화방 · 함께 보던 궁합)에는 닉네임 대신 **`탈퇴한 사용자`** 가 선다. **지금 처분은
그 자리를 남기지 않는다** — Match 와 대화방이 계정을 따라 cascade 로 함께 지워져(ADR 0023 ·
0091, runbook) 탈퇴한 사용자가 서는 화면이 아직 없다. 남길 것을 정하면 그때 선다(G-27).
...
**이 칸이 유일한 출처**다(ADR 0091). 닫힌 방 안의 안내 문구 넷은 `docs/prd.md` §7.1 의 표가 든다.
