...
**잠그지 않은 것.** `deny` 는 `cd x && git push --force-with-lease` 를 **막지 않았다** — 2026-09-23 에 bypass 모드의 세션이 작업 가지에 그렇게 밀었고 그대로 나갔다(G-50 PR). 막는다고 믿지 않는다 — 가지를 main 위로 옮길 때는 rebase 대신 main 을 merge 해 force push 가 필요 없게 한다. `ask` 는 bypass 모드에서도
