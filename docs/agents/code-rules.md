...
- **코드가 용어집과 다른 말을 쓰는 자리 둘**은 `CONTEXT.md` §10 「어긋난 이름」이 든다
  (옛 이름의 겉 `add_person_revision` · `DiscoveryProfile`, 그리고 「그대로 둔다」로 정한 `metaphor` · 후보 목록의 RPC 이름 · 사유값 `unreadable-revision` · 탈퇴 대기). 그 표의 이름이 코드에 아직 있는지는 시험이 잰다(ADR 0088).
