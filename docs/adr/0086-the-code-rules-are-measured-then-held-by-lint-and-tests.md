...
## 정정 (2026-09-22, 같은 날) — 「같은 뜻의 다른 표기」 셋이 또 있었다

머지 직후 검토가 우회 셋을 잡았고, 셋 다 재현됐다. ADR 0085 가 세 판을 지나며 남긴 교훈을 이
ADR 이 첫 판에서 그대로 되풀이했다.

**1. `.jsx` 가 전부를 지나갔다.** `tsconfig` 가 `allowJs` 라 Next 는 `.jsx` 를 그대로 컴파일하는데
린트의 `ANY` 도, 두 시험의 `SOURCE_EXTENSIONS` 도 `.jsx` 가 없었다. 코드 규칙만이 아니라
**ADR 0085 의 층 규칙도** 같은 구멍이었다 — `src/lib/x.jsx` 가 `react` 를 부르고 `app/x.jsx` 가
`.rpc()` 를 불러도 린트와 층 시험이 초록이었다. 목록 셋에 `jsx` 를 넣고 파서가 `ScriptKind.JSX`
로 읽는다. 린트의 glob 도 갈랐다 — 화면은 `{tsx,jsx}`, 화면 아닌 코드는 `{ts,mts,cts,js,mjs,cjs}`
(`.js` 도 `allowJs` 로 컴파일되므로 도메인 lib 의 패키지 제한이 이제 `.js` 에도 건다). 존재를
금지하는 길은 안 골랐다 — 금지 목록이 하나 더 생기는 일이다.

**2. 루트 설정 파일의 ADR 참조를 안 봤다.** `REFERRING_FILES` 가 루트에서는 `eslint.config.mjs`
와 `proxy.ts` 만 들었고, `playwright.config.ts` 에 없는 번호를 적어도 열일곱이 다 초록이었다.
루트의 소스 확장자 파일 전부를 든다.

**3. 허용된 중간 이름 앞에 아무 이름이나 숨을 수 있었다.** 정규식이 마지막 한 칸만 잡아
`x.weird.live.test.ts` 가 `live` 로 지나갔다. 기본 이름 뒤 점 구간 **전체**를 떼어 「비었거나 넷 중
정확히 하나」로 잰다.

반례 넷(`src/lib/input/probe.jsx` · `app/probe-screen.jsx` · 루트의 없는 ADR 번호 ·
`probe.weird.live.test.ts`)으로 **린트 오류 다섯, 시험 단언 여섯**이 빨개지는 것을 보고 지웠다.

## 앞선 결정과의 관계

- **ADR 0085** — 같은 방법(재고 → 잠그고 → 어겨서 증명)이고 `CODE_SHAPE` 는 그 블록들 위에
  얹혔다. 「예외는 이름이 말한다」도 그대로다. 정정에서 그쪽의 `.jsx` 구멍도 함께 막았다
