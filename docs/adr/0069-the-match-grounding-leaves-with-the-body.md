...
마이그레이션을 올린 뒤 대시보드에서 역할을 바꿔 가며 읽었다(읽기만 하고 되돌렸다). 대상은 **옛 컷으로 쓴
`reading-prompt-v8` 풀이** — 저장된 원문이 `### 근거` 절과 `compatibility.eokbuMatch` 인용을 실제로 들고 있다.

| 누구 | 본문 행 | 본문에 근거 절 | 원문 행 | 근거 자료 행 |
| --- | --- | --- | --- | --- |
| 당사자 (운영자 아님) | 1 | 없음 | 0 | 0 |
| 당사자 (운영자) | 1 | 없음 | 1 | 1 |
| 무관한 사용자 | 0 | — | 0 | 0 |

**저장된 원문은 그대로다**(`근거절 true · eokbu 인용 true`). 익명은 세 함수 모두 `42501` 이고,
`match_reading_source` 는 적용 전 `404`(PGRST202)에서 적용 후 `42501` 로 갈렸다.
