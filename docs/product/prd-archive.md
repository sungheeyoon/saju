> **US 번호의 출처. 요구사항은 `docs/prd.md` 가 축이다 — 이 문서를 요구사항으로 읽지 마라.**
>
> 코드와 SQL 주석이 가리키는 `US 23-1`·`US 29`·`US 43`·`US 44`·`US 56`·`US 59`·`US 64`
> 같은 번호가 여기 있어서 남긴 것이다. 2026-09-03 감사 이후 제품이 무엇을 하는지는
> `docs/prd.md` 가 답하고, 어긋나면 그쪽이 맞다.

# PRD: 동의 기반 사주 매칭 서비스

- 상태: 구현 준비
- 제품 단계: 폐쇄형 초대 MVP
- 기준 문서: 루트 도메인 용어집과 ADR 0001~0024
...
- **Discovery는 맛보기다(2026-08-25 확정, 2026-09-15 개정).** 후보 카드는 내게 없거나
  적은 오행 중 후보가 가진 것의 이름을 설명한다. 오행 하나로 성장·표현 같은 효과까지
  단정하지 않는다.
...
- 후보 카드에는 두 축의 가중합을 `오행 첫인상 점수`로 노출한다. 오행 구성을 단순 비교한 참고
  점수라고 바로 설명하고, 상세 궁합의 AI 점수와 구분한다. 두 축의 원값은 노출하지 않는다.
- 참여 화면은 켜기 전에 다음을 그대로 알린다. 상대에게 보이는 것은 공개용 별명과 소개,
  나에게 없거나 적은 오행 중 후보가 가진 오행의 이름, 함께 놓았을 때의 오행 균형을
  말로 옮긴 설명이다. 보이지 않는 것은 생년월일시와 출생지, 여덟 글자·상대 원국 판정과
  전체 오행 개수표다. 오행 첫인상 점수는 상대에게 보인다. 화면·ADR·PRD는 같은 문장을 사용한다.
