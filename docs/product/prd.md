...
- `match-v0` 계산 정책도 실험 중이다. 숫자가 바뀌어도 되며 Match 성립 시점의 지표
  snapshot을 따로 만들지 않는다. 공유 결과는 열 때마다 현재 정책으로 다시 계산하고, 화면은
  그 값이 어느 정책 버전의 것인지 값과 함께 든다. 가중치를 바꾸면 정책 버전 문자열도
  함께 올린다 — 같은 버전 이름으로 다른 숫자가 나가면 버전이 거짓말을 한다.
