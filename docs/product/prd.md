...
- Match 궁합 점수는 결과가 처음 만들어질 때 한 번 계산해 저장하고, 양쪽 User에게 같은
  저장값을 보여 준다. AI는 이 점수를 설명할 뿐 새 점수를 만들지 못한다.
...
- AI 해석은 사용자가 명시적으로 요청할 때만 생성되고, 세 kind의 접근 범위가 서로를 열지 않는다.
...
25. As an User, I want to 최근 Reading 목록을 빠르게 본다, so that 큰 Evidence 전체를 내려받거나 파싱하지 않고 원하는 결과를 찾을 수 있다. (9단계 범위 밖 — 데이터는 그때 쌓이고 화면은 뒤에 온다.)
...
47. As an Match 당사자, I want to 두 사람 사이의 궁합 사실과 저장된 `match-v0` 결과를 함께 본다, so that 다시 열거나 계산 정책이 바뀌어도 양쪽이 처음 받은 같은 점수를 본다.
...
53. As an Match 당사자, I want to AI가 점수를 새로 만들지 않고 Match에 저장된 `match-v0`만 설명한다, so that 코드가 만든 점수와 AI가 만든 다른 점수가 한 화면에서 경쟁하지 않는다.
...
- Reading 모듈은 redacted Evidence 생성, 지표 값과 정책 버전 동봉, prompt 조립, AI 호출,
  출력 검사, 저장과 사용자별 응답 projection을 하나의 파이프라인으로 감싼다. 세 kind는
...
- Match 궁합 결과가 처음 만들어질 때 매인 chart revision으로 `match-v0`를 한 번 계산해
  **점수만** 저장한다. 저장하는 것은 종합 점수, 네 축의 항목별 점수, 내부 정책 버전
  셋뿐이다. 양쪽 UI는 이후 이 점수를 다시 산출하지 않고 저장값을 읽는다. 정책 버전은
  내부 감사·재현용이며 사용자에게는 점수와 「실험 중인 지표」라는 고지만 보여 준다.
  계산식 변경은 새로 결과가 만들어지는 Match에만 적용된다.
- **관계 사실은 저장하지 않는다.** 형충회합을 비롯한 궁합 사실은 Match에 매인 revision을
  최신 엔진으로 그때그때 계산해 보여준다. 사실은 사실이므로 엔진 버그를 고쳤으면 고쳐진
  값이 맞고, 틀린 사실을 과거 상태로 보존할 이유가 없다. 화면 전체를 굳히는 snapshot은
  만들지 않는다 — 고정하는 것은 사용자에게 한 번 제시된 점수다.
- 저장된 점수는 관계 계산이 바뀌어도, 점수 정책이 바뀌어도 다시 산출하거나 덮어쓰지
  않는다. 과거 Match를 그리려면 그 Match가 저장한 정책 버전의 축 정의가 코드에 남아
  있어야 한다. 새 정책을 들일 때 이전 버전의 축 정의를 지우지 않는다.
- Reading은 `kind`, 대상 Person 한 명 또는 두 명, 해당 chart revision, 생성 요청자 또는
  Match, `viewedAt`, 정확한 redacted Evidence 문자열, prompt version, 모델·provider·생성
  파라미터, 출력과 생성 시각을 보존한다. 두 사람을 대상으로 할 때는 그때의 `match-v0`
  값과 정책 버전을 함께 보존한다.
...
- 자기 명식과 직접 관리하는 Person의 수동 궁합은 현재 revision에서 필요할 때 계산한다.
  ...
  수 있음을 알린다. Match는 매인 판본과 최초 생성 때 저장한 궁합 점수를, Reading은 당시
  Evidence와 전달받은 점수를 사용하므로 현재 입력·엔진·점수 정책 변경이 이미 제공된 결과를
  조용히 바꾸지 않는다.
...
- `COMPAT_POLICY.scoring`은 계속 `not-scored`다. 엔진은 사실만 내고 `match-v0`는 엔진 밖
  제품 정책으로 버전을 갖는다.
- Match 궁합 결과는 처음 생성할 때의 `match-v0` 종합 점수와 네 축 항목별 점수, 그리고
  내부 정책 버전을 저장한다. 과거 Match는 화면을 다시 열어도 이 점수를 다시 계산하지
  않는다. 저장 대상은 점수뿐이고 관계 사실은 매인 revision으로 다시 계산한다. 계산식
  변경은 이후 새로 결과가 만들어지는 Match에만 적용한다. 정책 버전은 내부에 보존하되
  사용자 UI는 버전을 구분해 설명하지 않고 저장된 점수와 실험 중이라는 고지만 보여 준다.
...
- **AI는 사용자의 명시적 요청 한 번에 한 번만 생성한다.** 화면을 열 때마다 다시 호출하지
  않는다. 이미 생성된 Reading이 있으면 그것을 보여주고, 새로 만들려면 사용자가 다시 요청해야
  한다. 비용과 지연이 사용자가 안 시킨 자리에서 발생하지 않게 하려는 것이고, 같은 근거로
  같은 화면이 매번 다른 글을 내지 않게 하려는 것이다.
- 이 단계의 프롬프트는 **확정된 자산이 아니라 실험 대상**이다. 기존 프롬프트를 그대로
  고정하지 않고 versioned 실험 프롬프트로 시작한다. 무엇을 보여줄지가 아직 바뀌므로 화면은
  프롬프트의 출력 구조를 알지 않는다 — 출력은 원문 Markdown 그대로 세우고, 섹션을 뜯어
  카드로 만들지 않는다. 그래야 프롬프트를 고쳐도 화면이 깨지지 않는다.
- `match-v0` 계산 정책도 실험 중이다. 계산식은 계속 바꿀 수 있지만 변경된 공식은 이후 새로
  궁합 결과가 만들어지는 Match에만 적용한다. 이미 저장된 Match 점수는 다시 계산하거나
  덮어쓰지 않으며 양쪽 UI는 같은 저장값을 읽는다. 정책 버전은 내부 재현용 메타데이터이고
  사용자에게는 노출하지 않는다.
- 이 단계의 화면은 버튼, 원문 Markdown, 생성 상태만 제공한다. Evidence JSON·실제 프롬프트·
  출력 검사 결과는 **내부 테스트 화면에서만** 본다. 최근 Reading 목록과 과거 결과 관리
  화면(US 25)은 이 단계에서 사용자에게 제공하지 않는다. 데이터는 저장하되 화면은 뒤로 민다.
...
- AI는 중립 문체로 두 사람 사이의 양방향 사실을 함께 설명한다. `perspectivePersonId`에 따라
  근거나 결론을 바꾸지 않는다.
- AI는 점수·등급·승패·누구의 득실을 새로 만들지 않는다. 두 사람 Reading에서는 계산 시점에
  전달받은 `match-v0` 항목을 설명할 수만 있고, Match Reading은 Match에 저장된 점수를 쓴다.
...
- AI 호출은 인증, kind별 접근 권한, rate limit, idempotency key를 통과해야 한다. 네트워크
  재시도는 같은 생성 요청을 중복 Reading으로 만들지 않는다.
...
- discovery 분포 테스트는 고정 표본에서 출생시간 미상 여부, 특정 오행 분포와 후보 집단에
  ...
  자동 변경하지 않는다.
- `match-v0` 회귀 테스트는 네 축·가중치·제외 항목·0~100 범위와 내부 정책 버전을 검증한다.
  Match 결과가 한 번 저장된 뒤에는 계산 정책을 바꾸거나 화면을 다시 열어도 점수가 바뀌지
  않고, 새 정책은 이후 새로 생성된 결과에만 적용되며, AI 출력이 저장값을 변경하지 못하는지도
  검증한다.
...
- Reading 테스트는 세 kind의 접근 범위가 서로를 열지 않는지, 그리고 화면을 여는 것만으로는
  생성이 일어나지 않고 명시적 요청에만 한 번 생성되는지를 검증한다.
...
9. redacted Evidence와 Reading 파이프라인을 **실험 인프라로** 만든다. 해석 완성 단계가
   아니다. 세 kind(자기 풀이·비공개 궁합·Match 궁합)를 각자의 접근 범위로 세우고, 명시적
   요청 한 번에 한 번 생성하며, 당시 Evidence·프롬프트·출력·점수·버전을 저장한다. 프롬프트는
   확정하지 않고 versioned 실험 프롬프트로 시작해 내부 품질 게이트를 반복한다.
