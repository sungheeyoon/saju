...
- Match 궁합 결과가 처음 만들어질 때 매인 chart revision으로 `match-v0`를 한 번 계산해
  종합 점수와 항목별 점수를 저장한다. 양쪽 UI는 이후 이 저장값만 읽고 다시 계산하지 않는다.
  정책 버전은 내부 재현을 위해 함께 저장하지만 사용자에게는 점수와 「실험 중인 지표」라는
  고지만 보여 준다. 계산식 변경은 새로 결과가 만들어지는 Match에만 적용된다.
...
- `COMPAT_POLICY.scoring`은 계속 `not-scored`다. 엔진은 사실만 내고 `match-v0`는 엔진 밖
  제품 정책으로 버전을 갖는다.
- Match 궁합 결과는 처음 생성할 때의 `match-v0` 종합 점수와 항목별 점수를 저장한다. 과거
  Match는 화면을 다시 열어도 이 값을 다시 계산하지 않는다. 계산식 변경은 이후 새로 결과가
  만들어지는 Match에만 적용한다. 정책 버전은 내부에 보존하되 사용자 UI는 버전을 구분해
  설명하지 않고 저장된 점수와 실험 중이라는 고지만 보여 준다.
