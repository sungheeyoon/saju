 ...
 *
 * 바꿔도 파장은 23:00~24:00 출생에 갇힌다. 2026-08-15 에 뒤집어 보니 골든 스냅샷에서 두 건만
 * 변했다.
 */
...
/**
 * 시간 미상일 때 조자시 경계는 **알리지 않기로 했다** — 빠뜨린 것이 아니다.
 *
 * 기본값인 조자시는 일주 경계가 23:00 이다. 그래서 늦은 23시대에 태어난 사람은
 * 일주가 하루 뒤여야 하는데, 시각을 모르면 우리는 그날 일주를 준다. 일간이 바뀌면
 * 십성·뿌리·강약·억부·조후(일간이 표를 여는 열쇠다)·종격이 전부 따라 바뀐다.
 *
 * 그런데도 알리지 않는다. 경도 보정(-32분 안팎)까지 걸면 실제로 어긋나는 창이
 * **하루 중 28분 남짓**이고, 절입일 경고처럼 "이 날에 걸렸다"고 좁혀 말할 수도
 * 없어 모든 시간 미상 명식에 "일주가 틀릴 수도 있다"를 붙이게 된다. 매번 붙는
 * 경고는 읽히지 않고, 읽히지 않는 경고는 절입일 경고까지 함께 무디게 만든다.
 * 시각을 넣지 않은 쪽이 지는 몫으로 둔다.
 *
 * 되돌린다면 조건은 하나다 — **그 28분을 좁혀 말할 수 있게 될 때.** 야자시로
 * 계산하면 경계가 자정이라 이 어긋남 자체가 없다.
 */
export const LATE_NIGHT_UNKNOWN_HOUR = 'not-warned';
...
function collectWarnings(
  ...
  if (!hourKnown) {
    ...
    // 아래 시지·자시 경계 경고는 여기서 끊긴다. 조자시 경계를 알리지 않는 것은
    // 판단이지 누락이 아니다 — 위 `LATE_NIGHT_UNKNOWN_HOUR` 에 이유를 적었다.
    return warnings;
  }
