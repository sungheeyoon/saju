 ...
 *
 * 1. **해석하지 않는다.** 문장도 점수도 길흉도 없다. 궁합 기준점(`previewScoreOf`)은
 *    엔진 밖 정책 모듈의 것이고 여기 오지 않는다.
 ...
 *
 * **두 자리가 아니라 네 자리다.** 두 자리로 끊으면 골든 명식에서 오행 비율이 두 건 더
 * 동점이 됐다(다섯 건 → 일곱 건, 2026-08-23 에 잰 값). 없는 동점을 만드는 것은 긴 숫자보다
