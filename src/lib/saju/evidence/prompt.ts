 ...
 */
const voiceWithRule3 = (rule3: string): string => `## 어떻게 말할 것인가
...
const VOICE = voiceWithRule3(
  '**문단마다 「그래서」로 닫아라.** 이런 사실이 있다 → 그것이 이 사람 하루에 어떻게 나타나는가 → 그래서 무엇을 하면 되는가. 셋째 칸이 없는 문단은 지워라.',
);

/** 내부 단일변수 실험판 — 판단 순서는 유지하고, 사용자에게 보이는 순서만 뒤집는다. */
const ANSWER_FIRST_VOICE = voiceWithRule3(
  '**절마다 첫 문장에 결론을 써라.** 판단은 아래 「성격을 읽는 순서」를 끝까지 거친 뒤 한다. 출력만 결론 → 왜 그런가 → 언제 어떻게 할 것인가 순서로 쓴다. 결론부터 쓴다고 앞의 근거 층을 건너뛰거나 신살 하나로 바로 답하지 마라.',
);
...
\`강점 — analysis.strength [유도] · analysis.rootQuality [유도]\`

자료 밖에서 가져온 말이 그 절에 있으면 \`자료 밖\` 이라고 함께 적어라. **가장 흔들리는 것 셋**을 마지막에 덧붙인다 — 무엇을 더 알면 단단해지는지까지. \`contract.excluded\` 와 \`limitations\` 도 여기서 읽는다.`;
...
export const PROMPT_PARTS = {
  ...
  /** 말투와 뼈대 */
  voice: VOICE,
  /** 내부 실험용 — 판단 순서는 그대로 두고 출력만 결론부터 */
  answerFirstVoice: ANSWER_FIRST_VOICE,
  /** 성격을 읽는 순서 — 신살이 성격을 정하지 않는다 */
