...
지금을 말할 때는 **원국의 무엇이 지금 건드려지는가**로 이어라. 운은 없던 성질을 만들지 않는다 — 있던 것을 세게 하거나 눌러 둘 뿐이다.
...
**층은 천장이다.** 그 층이 허락하는 것보다 세게 말하지 마라. 천장 아래에서 실제로 얼마나 세게 쓸지는 **근거가 몇 갈래로 모이는가**가 정한다 — 층이 \`사실\` 이어도 근거가 하나뿐이면 단정하지 않는다. 둘이 다른 답을 가리키면 **낮은 쪽**을 따른다. 딱지(\`[사실 · analysis.elements]\` 같은 것)는 **본문에 달지 말고 맨 끝 근거 칸에 모은다** — 문장마다 붙으면 글이 죽는다.`;
...
**층이 정한 천장 아래에서** 실제 강도를 정하는 것이 이것이다. 「~이다」라고 단정하는 것은 **서로 다른 근거 여럿이 같은 방향을 가리킬 때**뿐이다. 근거가 하나면 「~한 경향이 있다」·「~로 나타날 수 있다」로 쓴다. 층이 그보다 낮으면 층을 따른다 — 둘 중 **낮은 쪽**이 답이다.
