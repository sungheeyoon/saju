...
다만 층이 얕은 자리에서는 **말투로 그것이 드러나야 한다.** 단단한 것은 「…이다」로, 얕은 것은 「…쪽으로 보인다」·「…라면 이렇게 읽힌다」로 쓴다. 딱지(\`[사실 · analysis.elements]\` 같은 것)는 **본문에 달지 말고 맨 끝 근거 칸에 모은다** — 문장마다 붙으면 글이 죽는다.`;
...
「~이다」라고 단정하는 것은 **서로 다른 근거 여럿이 같은 방향을 가리킬 때**뿐이다. 근거가 하나면 「~한 경향이 있다」·「~로 나타날 수 있다」로 쓴다.
