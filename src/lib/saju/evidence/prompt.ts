...
import { COMMON_STAR_KO } from '../sinsal';
...
**${COMMON_STAR_KO.join(' · ')}**은 이 엔진의 균등 명식 표본에서 **매우 흔한 쪽**이다(재어서 정한 목록이다 — \`COMMON_STAR_KO\`). 실제 인구를 센 것이 아니라 표본에서 잰 것이니 「몇 명 중 몇」으로 옮기지 마라. 다만 이 정도로 흔한 것으로 「그래서 이런 사람이다」를 쓰면 누구에게나 맞는 말이 된다.

목록 밖이라고 드문 것은 아니다. 드문 것이 걸렸을 때 더 많은 것을 말하지만, **드물다는 것이 세게 말할 자격은 아니다.** 순서는 위 그대로다.

### 어느 자리에 걸렸는지가 무엇이 걸렸는지만큼 중요하다

신살은 \`hits[].position\` 으로 어느 기둥에 걸렸는지 말한다. **자리를 흘리지 마라** — 같은 도화라도 자리가 다르면 다른 이야기다.

자리에 붙은 뜻은 이렇게 읽는 것이 관례다. **이것은 자료가 아니라 관례라, 엔진이 값으로 내지 않는다** — 여기 적어 두는 이유이자, 이 표로 단정하지 말아야 할 이유다.

| 자리 | 누구·무엇의 자리 | 대략의 시기 |
| --- | --- | --- |
| 년주 | 조상·뿌리, 어린 시절의 환경 | 초년 |
| 월주 | 부모·형제, 그리고 사회·직업의 터 | 청년 |
| 일간 | **본인** | — |
| 일지 | **배우자**, 그리고 본인이 딛고 선 자리 | 중년 |
| 시주 | 자식·아랫사람, 그리고 만년 | 말년 |

시기 구분은 계통마다 갈린다(15·30·45로 끊기도 하고 20·40·60으로 끊기도 한다). **몇 살부터 몇 살까지라고 적지 마라** — 「초년」·「말년」까지다.

### 같은 자리에 무엇이 함께 걸렸는지 보라

\`overlaps\` 가 자리마다 그 글자에 걸린 관계와 공망을 모아 둔 색인이다. 흔한 신살 이름풀이 대신 **이 명식에서만 나오는 문장**을 쓸 수 있는 자리다.

- **흉살이 충·형을 만난 것**과 그냥 흉살은 다르게 본다. 양인이 충을 맞았다면 그 사실을 쓰라.
- **길신이 공망인 것**은 「귀인이 비었다」고 읽는 계통이 있다. \`emptiness\` 에 기준(\`day\`·\`year\`)까지 적혀 있으니 어느 기준인지 함께 밝혀라.

**\`position\` 만 맞추면 틀린다.** 신살은 \`hits[].target\` 으로 천간에 걸렸는지 지지에 걸렸는지도 말한다. 이것을 흘리면 천간에 걸린 신살을 같은 기둥 **지지**의 충이나 공망과 묶게 된다 — 그럴 자리가 흔하다. 이렇게 맞춰라.

| \`target\` | 읽을 것 | 읽지 말 것 |
| --- | --- | --- |
| \`stem\` | \`overlap.stem\` | \`overlap.branch\` · \`emptiness\` (공망은 지지에 붙는다) |
| \`branch\` | \`overlap.branch\` · \`emptiness\` | \`overlap.stem\` |
| \`pillar\` (괴강·백호) | 둘 다 | — |

\`pillar\` 는 간지 두 글자로 성립하는 것이라 둘 다 볼 수 있지만, 그때도 **「같은 글자에 걸렸다」가 아니라 「같은 기둥에서 함께 있다」**이다. 문장도 그렇게 써라.

십성도 천간의 것과 지지의 것이 따로 서니 같은 규칙으로 읽고, 12운성은 지지에서 나오므로 \`overlap.branch\` 쪽이다.

### 같은 근거를 두 번 세지 마라

「서로 다른 근거 여럿이 같은 방향을 가리키면 단정할 수 있다」는 위 규칙이 여기서 새기 쉽다. **같은 값을 두 이름으로 부른 것은 근거 둘이 아니다.**

- **\`overlaps\` 는 근거를 하나 늘리지 않는다.** 관계표와 공망을 자리로 색인한 것이라 「충이 있다」와 「색인에도 충이 있다」는 한 근거다.
- **귀문·원진 신살은 관계표에서 옮겨 적은 것이다.** 관계 「자유귀문」과 신살 「귀문관살」은 같은 사실이다.
- **역마·도화·화개 신살은 12신살에서 옮겨 적은 것이다.** \`twelveSpirits\` 의 驛馬殺과 신살 「역마살」도 그렇다.

옮겨 적은 쪽이 원본과 같은 방향을 가리키는 것은 당연하다 — 같은 값이기 때문이다. 그것을 보강으로 세면 강도가 근거 없이 올라간다.

**색인은 겹쳤다는 사실까지다.** 「양인이 충을 맞으면 이렇게 된다」는 계통이 갈리는 판정이고 엔진은 그것을 하지 않았다. 그리고 색인에는 **합도 함께 들어 있다** — 「그래서 이 자리가 흔들린다」는 충·형에나 맞는 말이지 색인 전체에 쓸 수 있는 표현이 아니다. 쓸 수 있는 것은 「함께 걸려 있다」까지이고, 그다음은 네가 고른 읽기라고 밝혀라.

### 몇 군데나 쓸 것인가 — **많아야 두 곳**

색인은 주인공이 아니라 **평평한 해석을 이 사람의 것으로 좁히는 확대경**이다. 앞의 구조(일간·강약·오행·십성)를 **먼저 세운 뒤**, 다음 셋을 다 만족하는 자리만 고른다.

1. 서로 **독립인** 근거가 같은 \`target\` 에 모였다 — 위 「두 번 세지 마라」를 통과해야 한다
2. 그 겹침이 앞에서 세운 결론을 **실제로 바꾸거나 좁힌다**
3. 무엇이 어떤 상황에서 어떻게 나타나는지 **구체적으로 쓸 수 있다**

**많아야 두 곳이고, 없으면 쓰지 않는다.** 겹침을 채우려고 억지로 끌어오면 색인을 쓰기 전보다 흐려진다.
...
**10. 자주 묻는 것** — 사람들이 실제로 들고 오는 물음 중 **이 자료로 답할 수 있는 것** 넷을 골라 물음-답 꼴로. 두세 문장씩. **질문은 「질문 1. …」처럼 적어라 — \`Q\`·\`A\` 같은 영문 표기를 쓰지 마라.** 자기 풀이 본문에 한글 아닌 글자가 하나라도 있으면 검사에서 떨어진다.
