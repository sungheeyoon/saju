...
/** 용신을 어떤 길로 잡았는가 */
export type YongsinMethod = 'eokbu';

export const YONGSIN_METHOD_KO: Record<YongsinMethod, string> = {
  eokbu: '억부용신',
};
...
export type Yongsin = {
  ...
  /** 그 오행이 일간에게 무엇인가 */
  role: ElementRole;
  /** 왜 이것을 골랐는가 — 화면에 그대로 쓸 수 있는 한 문장 */
  reason: string;
  /**
   * 용신을 극하는 오행. 꺼리는 쪽이다.
   *
   * 희신(용신을 돕는 오행)은 내지 않는다. 신약에 인성을 쓸 때 그것을 생하는
   * 관성이 이미 과다해서 문제인 경우가 흔해, 규칙 하나로 답하기 어렵다.
   */
  avoid: Element;
};
...
export function yongsinOf(
  ...
  const { scores } = elementDistributionOf(pillars, weights);
