...
describe('궁합 외부 사례 — 모양', () => {
  it('id 는 겹치지 않고 두 계통 · 서른한 쌍이다', () => {
    ...
    expect(byLineage).toEqual({
      'chinese-modern-consult': 28,
      'korean-modern-forum': 3,
    });
    // 같은 글쓴이의 글은 서로 독립이 아니다 — 글쓴이로도 센다.
    expect(new Set(COMPAT_EXTERNAL_CASES.map((c) => c.practitioner)).size).toBe(7);
  });
...
describe('궁합 외부 사례 — 전문가 판정과 주는 쪽 자리(관찰)', () => {
  ...
  it('판정이 선 방향은 58 이고 「채운다」가 36 · 「해친다」가 16 이다', () => {
    expect(MEASURED).toHaveLength(58);
   ...
   * 받는 쪽이 바란 오행들 중 주는 쪽에서 가장 센 자리(`presences[0]` 의 최소).
   *
   * 「채운다」 36 방향은 **하나도** 숨은 자리(`hidden-only`)나 없음에서 서지 않았다 — 35 가 천간(일간 16 ·
   * 다른 천간 19)이다. 거꾸로 「해친다 · 못 채운다」 17 방향 중 14 도 바란 오행이 천간이나 본기에 드러나 있다.
  ...
  it('「채운다」는 모두 드러난 자리에서 섰고, 숨은 자리에만 있으면 「채운다」가 0 이다', () => {
    ...
    // 지장간을 공급 근거로 든 전문가는 58 방향에 한 번도 없다. 거꾸로 「천간에 안 드러났다(不现)」를
   ...
   *
   * 「채운다」의 가운데값은 4, 「해친다 · 못 채운다」는 2 다. 넷 이상이 「채운다」 36 중 31, 부정 17 중 4.
   * 전문가 35 방향이 「旺 · 多 · 林立」을 근거로 든 것과 같은 쪽이다 — 그들은 자리보다 **양**을 말한다.
   */
  it('바란 오행의 드러난 글자 수가 넷 이상인 방향이 「채운다」에 몰린다', () => {
    const atLeast = (rows: typeof MEASURED, n: number) =>
      rows.filter((m) => m.needVisible >= n).length;
    expect([atLeast(SUPPLIES, 3), atLeast(SUPPLIES, 4), SUPPLIES.length]).toEqual([33, 31, 36]);
    expect([atLeast(NEGATIVE, 3), atLeast(NEGATIVE, 4), NEGATIVE.length]).toEqual([7, 4, 17]);
  ...
  it('출처가 든 근거는 「왕하다(abundance)」가 35 로 가장 많고, 일간 · 일주를 든 것은 여덟이다', () => {
    ...
    expect(MEASURED.filter((m) => m.cited.length === 0)).toHaveLength(12);
  });

  /**
   * 엔진의 세 기준(ADR 0112)과 전문가 판정. `any` 는 58 중 57 이 `mixed` 라 판정과 무관하다.
   * `stems` 의 `supportive` 는 「채운다」 36 중 12, 「해친다」 16 중 1 이다.
   */
  it('엔진의 any 기준은 판정을 가르지 못하고 stems 기준만 조금 기운다', () => {
    ...
    expect(MEASURED.filter((m) => m.relation.any === 'mixed')).toHaveLength(57);
    ...
    // 엔진의 억부 1순위 오행이 출처가 적은 필요 오행 안에 드는 방향 — 필요 자체가 58 중 25 에서 어긋난다.
    expect(MEASURED.filter((m) => m.enginePrimaryInNeeds)).toHaveLength(33);
  });
});
