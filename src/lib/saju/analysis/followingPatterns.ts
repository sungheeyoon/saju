...
export const FOLLOWING_PATTERN_POLICY = {
  ...
  dominance: {
     ...
     *
     * 종격이라고 적힌 **서른셋 중 열여덟**을 종격 쪽으로 본다.
     * 올라간 셋은 문턱을 내려서 얻은 것이 아니다 — 문턱은 그대로 두고 세는 법을
     * 고쳤다. 같은 3000건 표본의 발화율이 10.9% 에서 10.6% 로 오히려 내려간 것이
     * 그 증거다.
     *
     * **셋째 계통을 넣고 성적이 나빠졌다.** 《천리명고》 평단편에서 여섯을 더했더니
     * 재현율은 17/30 → 18/33 으로 사실상 제자리이고 오검출은 **1 → 2** 로 늘었다.
     * 나빠진 것이 이 자료의 값이다 — 앞의 두 계통에만 맞던 자리가 드러났다는 뜻이고,
     * 계통을 섞는 이유가 정확히 그것이다.
     *
     * 늘어난 오검출 하나(`qlmg-xu-shiying`)는 **저자가 당대 통설을 반박하는 자리**다.
     * 「識者咸以從殺格推之」라고 적고 그것을 뒤집는데, 우리는 통설 쪽에 선다. 오검출로
     * 세되 그것이 계통 차이라는 것도 함께 읽어야 한다.
     *
     * 그런데도 `passed` 는 여전히 `false` 다. 발화율이 고전이 말하는 희소성과 자릿수가
     * 다르고, 아니라고 적힌 일곱 중 둘을 종격으로 본다. 자세한 행렬은
