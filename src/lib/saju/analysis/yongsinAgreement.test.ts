...
describe('억부와 조후의 대조', () => {
  ...
  /**
   * **어긋나는 쪽이 드물지 않다** — 이 대조를 세우는 이유다.
   *
   * 무작위 3000건(시드 20260821)에서 두 길이 같은 것을 가리키는 것은 56.8% 다.
   * 나머지 43.2% 에서는 화면의 두 칸이 서로 다른 것을 권하고 있었고, 그 사실을
   * 아무도 말하지 않았다. 「드문 예외라 안 다룬다」로 넘길 수 있는 자릿수가 아니다.
   */
  it('무작위 3000건에서 억부와 조후가 갈리는 비율을 재어 둔다', () => {
    const aligned = randomInputs(3000).filter(
      (input) => computeSaju(input).analysis.yongsinAgreement.aligned,
    ).length;

    expect(Math.round((aligned / 3000) * 1000) / 1000).toBe(0.568);
  });
});
