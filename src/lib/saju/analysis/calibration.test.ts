...
describe('모집단 보정값 (시드 20260821 · 3000건)', () => {
   ...
   * 40.9% 에서는 두 칸이 서로 다른 것을 권하고 있었고 아무도 그 사실을 말하지 않았다.
   *
   * 월지 ×2 · 지장간 60:30:10(ADR 0114) 전에는 0.568 이었다. 억부 1순위가 13.8% 바뀌면서 조후와
   * 만나는 쪽이 조금 늘었다.
   */
  it('대조 — 억부와 조후가 같은 것을 가리키는 비율', () => {
    expect(share(counted.aligned)).toBe(0.591);
  });
  ...
  it('서열 — 어긋남이 하나라도 있는 명식의 비율', () => {
    // ADR 0114 전에는 0.545 — 억부가 바뀌어도 어긋남이 있는 명식의 몫은 거의 그대로다.
    expect(share(counted.shaken)).toBe(0.542);
  });
