 ...
 *
 * 그래서 **격국은 억부를 뒤집지 않는다.** 종격과 같은 자리다 — 판정은 내되 상위에
 * 서지 않는다. 한동안 다른 점은 「격국에는 대조가 아직 없다」였는데, 이제 그것도
 * 잰다(`externalCheck`): 일흔둘 중 예순셋. **그래도 안 뒤집는다** — 계통이 하나뿐이고,
 * 남은 아홉이 겸격을 한 이름으로 접는 데서 오기 때문이다.
 *
 * ## 격을 잡는 법
 *
...
export const STRUCTURE_POLICY = {
   ...
   * 외부 대조 — **처음 재 봤다.**
   *
   * 《子平真詮》의 격국 장 여덟에서 명조 일흔넷을 모아 격의 종류를 대조했다. 일흔둘이
   * 실재 가능하고(둘은 판본 오배) 그중 **예순셋이 맞는다(87.5%).**
   *
   * 처음 재었을 때는 쉰여섯이었다. 올린 것은 문턱이 아니라 **출처의 조항**이다 —
   * 왕지 월령을 정기 하나로 보게 고쳤고(`peakMonth`), 그 근거는 이 성적이 아니라
   * 「除子午卯酉外，餘皆有藏」이라는 원문 한 줄이다.
   *
   * **그런데 게이트는 안 연다.** 두 가지 때문이다.
   *
   ...
   */
  externalCheck: {
    dataset: 'structureExternalCases',
    cases: 74,
...
/**
 * 성패의 조건이 낼 수 있는 이름 전부 — **문장 그물이 읽는 정적 목록.**
 *
 * 아래 `switch` 안에 흩어져 있어서 정적으로는 어디에도 모여 있지 않았고, 그래서
 * `MYEONGRI_LEXICON` 이 통째로 지나쳤다 — 「상관견관이 걸립니다」 같은 문장이 근거
 * 없이 그물을 빠져나간다는 뜻이다. 국 이름(`BUREAU_NAMES`)에서 이미 같은 구멍을
 * 메웠고 여기가 같은 자리다.
 *
 * 스무 개 중 둘(`식신제살`·`관인상생`)은 **금지 표현이기도 하다**. 그쪽은 그물이
 * 아니라 `FORBIDDEN_CLAIMS` 가 잡고, 그 명식이 실제로 그 조건을 낸 때만 열린다.
 *
 * 목록이 코드와 어긋나지 않는 것은 시험이 지킨다(`structure.test.ts`) — 여기
 * 적힌 이름과 판정이 실제로 내는 이름이 같은 집합이어야 한다.
 */
export const STRUCTURE_FACTOR_NAMES: readonly string[] = [
  '월령충파',
  '관봉재인',
  '상관견관',
  '관살혼잡',
...
/**
 * 왕지(旺支) — **격을 잡을 때는 지장간이 없다.**
 *
 * 출처가 이것을 조항으로 못박는다: 「如十二支中，**除子午卯酉外，餘皆有藏**，不必四庫也」
 * (《子平真詮》論用神變化). 열두 지지 가운데 이 넷만 藏이 없다는 말이다.
 *
 * **사령 표를 갈아 끼우지 않는다.** `HIDDEN_STEMS` 의 子에 壬 10일이 있는 것은 사령
 * 이론의 표준이고, 오행 점수·뿌리·강약·억부·종격·통관이 전부 그 표에서 나온다. 격을
 * 잡는 규칙 하나 때문에 그 표를 건드리면 이 저장소의 판정 전부가 함께 움직인다 —
 * 고치는 근거는 격을 잡는 조항이므로, 움직이는 자리도 격을 잡는 자리 하나여야 한다.
 *
 * 이 조항을 안 지키는 동안 卯월에 甲이 투간하면 상관격으로 잡았다. 출처는 같은 명조를
...
function selectSource(pillars: StructureInput) {
  ...
  const hiddens = PEAK_BRANCHES.includes(monthBranch)
    ? HIDDEN_STEMS[monthBranch].filter((hidden) => hidden.role === '正氣')
    : HIDDEN_STEMS[monthBranch];
