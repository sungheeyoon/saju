 ...
 * 잰다(`externalCheck`): 일흔둘 중 쉰여섯. **그래도 안 뒤집는다** — 계통이 하나뿐이고
 * 어긋난 열여섯이 흩어진 오차가 아니라 규칙 차이 두 무리이기 때문이다.
 *
 * ## 격을 잡는 법
 *
...
export const STRUCTURE_POLICY = {
   ...
   * 일흔둘 중 쉰여섯이 맞는데 **계통이 하나뿐이고 어긋난 자리가 규칙 차이로 모인다.**
   * 아래 `externalCheck` 가 그 둘을 값으로 든다.
   */
   ...
   * 실재 가능하고(둘은 판본 오배) 그중 **쉰여섯이 맞는다(77.8%).**
   *
   ...
   *    얼마나 맞는가」뿐이다.
   * 2. **어긋난 열여섯이 흩어진 오차가 아니라 규칙 차이 두 무리다.** 왕지 월령에서 여기가
   *    투출한 자리(식신↔상관이 뒤집힌다)와, 정기가 투출하지 않은 자리(「乙用申官」처럼
   *    저쪽은 정기를 격으로 부른다). 규칙이 갈리는 것을 알면서 그 판정으로 억부를 뒤집을
   *    수는 없다.
   *
   * 성적을 올리려고 규칙을 만지지 않는다 — 그러면 이 대조가 채점이 아니라 자기 답안지가
   * 된다. 고칠 근거는 원문의 규칙 조항이어야 하고, 그것은 별건이다.
   */
  externalCheck: {
