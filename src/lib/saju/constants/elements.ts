...
/**
 * 오행을 **그림이 보이는 말로** — `ELEMENT_KO` 를 밀어내지 않는다.
 *
 * 「목·화·토·금·수」는 셀 때 자연스럽다(「금이 셋이에요」). 그림을 그릴 때는 아니다 —
 * 「금이 목을 극한다」는 한자어를 한글로 적은 것일 뿐이고, 「쇠가 나무를 자른다」라야
 * 오행끼리 무엇을 하는지가 **낱말만으로 보인다.**
 *
 * 그래서 두 벌이 함께 산다. **어느 쪽으로도 못박지 않는다** — 자리에 맞는 쪽을 고르는
 * 일은 문장을 쓰는 쪽이 한다. 여기서 하는 일은 고를 것을 둘 다 내주는 것까지다.
 */
export const ELEMENT_PICTURE_KO: Record<Element, string> = {
  木: '나무',
  火: '불',
  土: '흙',
  金: '쇠',
