...
describe('시간 미상 절입일 경고 — 자정 부근', () => {
  ...
  /**
   * **비용이 정해진 시험이라 제 시간을 든다.**
   *
   * 201년 × 24절기를 전수로 훑는다. 기본 5초에 아슬아슬하게 들어가던 자리인데
   * (`vitest.config.mts` 가 그렇게 적어 두었다), 시험 파일이 늘어 러너가 붐빌 때마다
   * **이 시험이 먼저 넘어갔다** — 실패가 이 시험의 잘못이 아니라 옆자리 부하다.
   *
   * 표본을 줄여 빨리 만들 수도 있지만, 이 시험이 잡는 것은 **한 해라도 어긋나는가**라
   * 줄이는 순간 무엇을 재는지가 달라진다. 그러니 전수는 그대로 두고 시간만 든다.
   */
