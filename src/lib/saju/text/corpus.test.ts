...
describe('말뭉치', () => {
  ...
  describe('변종은 갈릴 근거가 있을 때만 가른다', () => {
    ...
    it('관계 종류는 변종이 아니라 슬롯이다', () => {
      ...
      // 남은 변종 셋은 종류가 아니라 **성립한 방식과 흔들림**이 가른다. 두 사람
      // 글자가 합쳐 이룬 것은 인정 여부 자체가 계통 선택이라 섞으면 안 되고,
      // 시주를 모르는 반쪽 합은 시주가 붙으면 흡수돼 사라질 수 있어 같은 문장을
      // 쓸 수 없다(`absorbableByUnknownHour`).
      expect(FRAGMENT_TOPICS['relation.present'].variants).toEqual([
        'row',
        'combined',
        'row-absorbable',
      ]);
