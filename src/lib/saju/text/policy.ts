 ...
 * | "이것이 전부다" | 말할 수 없다 → 한 칸 내린다 | 궁합 관계 목록 |
 * | 시주와 무관 | 그대로 | 조후(일간·월지만 본다) |
 *
 * 셋째 줄이 궁합에서 **한쪽만 시각을 모르든 둘 다 모르든 같은 칸**인 이유다.
 * 못 본 글자가 둘이냐 넷이냐는 정도 차이이고, "이 목록이 전부다"라고 말할 수
 * 없다는 점에서는 같다.
 *
 * **아직 원칙과 구현이 어긋나는 자리가 하나 있다.** 셋째 줄의 한계는 목록의
 * 것인데 지금은 **행 하나하나가 나눠 지고 있다** — L3 에 "목록"이라는 발화가
 * 없어서, 강도를 들 수 있는 것이 `Utterance` 하나뿐이기 때문이다. 행을 떼어 봐도
 * 단서가 붙어 있다는 실익은 있지만 원칙대로면 행은 `fact` 로 남아야 한다.
 * 목록 단위 발화가 생기면 그때 옮길 자리다.
 */
 ...
 * - **강등돼서**: 관계 행은 `fact` 에서 한 칸 내려와 `derived` 에 앉는다.
 *   시주가 빠져도 **그 합이 성립한다는 것은 그대로 참이고**, 내려간 것은
 *   "이 목록이 전부다"라는 쪽이다(`INCOMPLETE_INPUT_RULE`).
 *
 * 그래서 "을경합금(유도)"을 **합인지 아닌지가 우리 해석이라는 뜻으로 읽으면
 * 안 된다.** 합은 표에서 그대로 읽은 것이고, 유보는 목록에 걸려 있다.
 */
 ...
 * 2. **목록이 덜 나온다** — 관계는 적힌 한 줄 한 줄이 여전히 참인데 시주 두 글자가
 *    낼 관계를 통째로 못 봤다. 흔들리는 것은 항목이 아니라 **전체성**이다.
 * 3. **없다는 주장이 뒤집힌다** — 무근·종격. 이쪽은 내리는 것으로 부족해서
 *    `polarity: 'absence'` 가 통째로 잠근다(아래 `ceilingFor`).
 *
 * 셋을 값으로 갈라 적지 않은 것은 **갈라도 결과가 같기 때문이다** — 축을 하나 더
 * 만들어 놓고 아무 문장도 가르지 못하면 그 축은 장식이 된다(`FRAGMENT_POLICY`의
 * `variants: 'must-change-some-sentence'` 와 같은 판단). 셋이 서로 다르게 움직여야
 * 할 일이 생기면 그때 갈린다.
 */
