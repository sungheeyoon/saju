...
export const CLAIM_CEILING: Record<ClaimPath, ClaimStrength> = {
   ...
   *
   * 한동안 이 자리의 까닭은 「격국은 **대조가 아직 0건**」이었다. 이제 아니다 —
   * 일흔둘 중 쉰여섯이 맞는다. 그런데도 같은 칸에 앉는 까닭이 바뀌었을 뿐이다:
   * **계통이 하나뿐이고**(자평 계열에만 있는 눈금이라 섞을 자료가 없다) 어긋난 열여섯이
   * 흩어진 오차가 아니라 **규칙 차이 두 무리**다. 사다리에는 그 셋을 가르는 칸이 없으므로
   * 같은 `candidate` 에 앉히고, 무엇이 달라졌는지는 `STRUCTURE_POLICY.externalCheck` 가
   * 값으로 든다.
   */
