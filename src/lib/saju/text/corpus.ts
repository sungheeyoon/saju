...
const TRANSFORMATION_BODIES: Record<string, string> = {
   ...
   * **「하나」가 틀린 말이 되는 자리.**
   *
   * 한 글자를 둘이 물면 합도 둘로 세어져 같은 문장이 두 번 섰다 — 각각 「하나가
   * 걸려 있는데」라고 말하면서. 두 줄 다 참인 문장은 아니다: 걸린 것은 하나가
   * 아니고, 둘이 같은 글자를 두고 겹쳐 있다는 것이 이 자리의 사실이다.
   *
   * 이 변종만 化를 막은 이유를 말한다. **다른 이유들과 성질이 다르기 때문이다** —
   * 나머지 블로커는 「왜 안 변했나」의 조건 목록이고(그래서 접지 않는다), 쟁합은
   * 그 앞에서 **문장의 수를 정하는 사실**이다. 세지 않으면 개수가 틀린다.
   */
  contested:
    '{sharers} 사이의 {name} — {over} 하나를 두고 겹쳐 있어(쟁합·투합) {verdict} 자리라 {element} 쪽 무게로는 세지 않은 것',
...
const structureFragments: Fragment[] = [
   ...
   * **한 이름만 말하면 나머지가 사라진다.**
   *
   * 출처는 같은 명조를 두 이름으로 부르는 자리를 조항으로 둘 든다 — 본격(§112
   * 「又有變之而不失本格者」)과 묘고(§143 「辰…為水庫」). 어느 쪽이 이 명식의 격인지는
   * 우리가 판정하지 않기로 했으므로(`STRUCTURE_POLICY.nativeKind`), 문장도 **나란히
   * 적고 고르지 않는다.** 고르는 순간 그 문장은 우리가 안 한 판정을 한 것이 된다.
   */
  {
    topic: 'structure.kind',
    ...
    strength: 'candidate',
    template:
      '월지 {monthBranch} 자리의 {sourceStem} 쪽이 {revealedAt} 자리의 천간에 드러나 있어 {kind} 쪽을 후보로 봅니다. 다만 같은 자리를 {nativeKind} 쪽으로 부르는 길도 있어, 어느 쪽으로 볼지는 정하지 않았습니다.',
  },
  {
    topic: 'structure.kind',
    variant: 'revealed-with-native',
    strength: 'reference',
    template:
      '시주를 빼고 세면 월지 {monthBranch} 자리의 {sourceStem} 쪽이 {revealedAt} 자리의 천간에 드러나 있어 {kind} 쪽을 참고할 수 있습니다. 같은 자리를 {nativeKind} 쪽으로 부르는 길도 있어 함께 봅니다.',
  },
  {
    topic: 'structure.kind',
 ...
 * 선다는 점이다.
 *
 * 경계를 여기서 밝히는 것은, 해가 입춘에서 달이 절입에서 갈린다는 것이 **이 달에
 * 대한 사실이 아니라 우리가 어떻게 짚었는가**라서 행이 아니라 목록의 몫이기
 * 때문이다 — 양력 1월에 열면 세운이 전 해로 나오는 것이 여기서 설명된다.
 *
 * **마지막 문장은 안 바뀐다는 것을 적는다.** 한동안 "이 화면을 다시 열면 그때의
 * 시각으로 다시 셉니다"였는데, 그것이 화면이 **스스로 갱신되는 것처럼** 읽혔다 —
 * 그러면 오래 열어 둔 탭을 신선한 것으로 착각하게 되고, 이 문장을 넣은 목적의
 * 정반대가 된다. 실제 규칙은 제출한 순간에 정해지고 다시 제출할 때까지 그대로다.
 *
 * 링크로 나눠 준 결과가 다른 '지금'을 보이는 문제는 **첫 문장이 이미 막는다** —
 * 기준 시각이 문장 안에 박혀 있으므로 어제 찍은 스크린샷도 어제라고 말한다.
 * 그것을 막는 것이 셋째 문장의 몫이었던 적은 없다.
 */
const AS_OF = [
  '{at} 기준으로 짚은 운입니다.',
  '해는 입춘에서, 달은 절입에서 갈립니다.',
  '다시 제출하기 전까지는 이 기준 시각이 바뀌지 않습니다.',
].join(' ');
 ...
 *
 * **두 번째로 좁아졌다.** 처음에는 "대운이 낀 관계는 아직 세지 않아"였고, 대운 칸이
 * 관계를 들면서 "세 칸이 저마다 원국과 걸리는 것만"이 됐다. 이제 세운·월운이 자기를
 * 감싼 대운까지 견주므로 그 말도 거짓이 됐다 — 고지가 좁아지는 것이 채워졌다는 증거다.
 *
 * 남은 것은 세지 않은 것이 아니라 **덜어 낸 것**이다. 한 해가 대운 경계를 넘으면 그
 * 해는 두 대운에 걸치는데, 이 카드는 한 순간을 말하므로 지금 도는 대운이 아닌 쪽을
 * 뺀다(`inDaeun`). 뺀 것은 세운 표에 그대로 있으므로 어디서 볼 수 있는지까지 적는다.
 */
const NOW_COVERAGE =
  '운끼리 걸리는 것은 지금 도는 대운을 기준으로만 셌습니다. 한 해나 한 달이 대운 경계를 넘으면 다른 쪽 대운과 걸리는 것은 이 목록에 없고, 세운·월운 표에 있습니다.';
