...
describe('쉬운 말 판에 남은 분류명을 센다', () => {
  it('분류명은 잡고 오행 이름은 잡지 않는다', () => {
    ...
    expect(plainTermsIn('편인이 강해서 혼자 파고듭니다.')).toEqual(['편인']);
    expect(plainTermsIn('타고난 자리에 정인이 둘이에요.')).toEqual(['정인']);
  ...
  it.each([
    ...
    '먼저 인사해 보세요.',
    '둘이 함께 세운 규칙이 오래갑니다.',
    // #189 — v14 비공개 궁합 원문에서 편인 · 정인으로 잡혔던 두 문장
    '처음에는 작은 불편인데, 설명하지 않고 넘기면 쌓여요.',
    '결정 속도와 말투 때문에 상한 감정인지 분리해야 해요.',
