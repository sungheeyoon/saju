...
export function measureMatchRun({
   ...
   *
   * 이름 앞에 **사실 번호 하나**(`R4` · `S2`)가 붙을 수 있다 — `relations R4 자축합토 [사실]`. 자리 색인의 번호이고
   * 프롬프트가 시킨 것이 아니라 모델이 그렇게 적는다(ADR 0099 추기). 허용하는 것은 그 토큰 하나뿐이다 —
   * `4` · `R` · `X4` 는 번호가 아니라 읽지 않는다.
   */
  const cited = [
    ...grounding.matchAll(
      /(?:relations|관계)\s+(?:[RS]\d+\s+)?([가-힣]+(?:\s(?:반방합|반합|삼형|삼합|방합|[가-힣]+방))?)(?=\s*(?:\d+건\s*)?\[)/g,
    ),
  ].map((m) => m[1]);
