 ...
 * 더한 효과」로 읽지 않는다.
 *
 * **입력만 견준 1라운드다** — 지시는 두 판 모두 옛 궁합 지시(`plain-v1`)다. 운영은 A 입력에 읽는 법 4판을
 * 얹은 `CONTROL` 이고, 이 목록의 어느 쪽도 운영 조립이 아니다. 2~5라운드 변형은 실험을 닫으며 지웠다 —
 * 그 프롬프트는 커밋 `e586152` 에서 재현한다.
 *
...
export const MATCH_INPUT_VARIANTS: readonly MatchInputVariant[] = [
  {
    id: 'match-limited-v1',
    label: 'A 제한형',
    assembly: { ...CONTROL, matchInput: 'limited-v1', pairReading: 'plain-v1' },
    addedEvidence: [],
    promptChanges: [],
  },
  {
    ...
    label: 'B 확장형 (실험 전용)',
    assembly: { ...CONTROL, matchInput: 'extended-v1', pairReading: 'plain-v1' },
    addedEvidence: [
