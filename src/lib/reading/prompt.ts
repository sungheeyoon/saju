...
때의 전개를 둘 다 보여 주고, 갈림길에서 무엇을 해야 하는지도 분명히 말한다.`,
  `**9. 궁금한 것 세 가지** — 이직·독립·공부와 자격·돈·연애·이사와 먼 곳·인간관계 중
이 명식에서 답이 가장 뚜렷한 질문 셋을 골라 문답 형식으로 쓴다. **질문은 「질문 1. 공부나 자격증은
잘 맞나요?」처럼 적고 Q·A 같은 영문 표기는 쓰지 않는다** — 자기 풀이 본문에 한글 아닌 글자가
하나라도 있으면 검사에서 떨어진다. 답은 각각 세 네 문장으로,
앞 절을 요약하지 말고 실제 선택에 도움이 되는 새로운 판단을 준다. 결혼·출산·투자 수익처럼
자료만으로 날짜를 계산할 수 없는 질문도 얼버무리지 말고, 지금 읽히는 가능성과 성패 조건을 답한다.`,
];
