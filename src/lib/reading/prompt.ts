...
/**
 * 비공개 궁합 10절이 **각자를 읽을 때 자료를 어떻게 쓰는가** — 아직 기준판은 `unranked` 다.
 *
 * - `unranked` — 지금 실제로 나가는 것. 어느 판정도 지목하지 않는다.
 * - `precedence-v1` — 갈린 판정의 서열(`analysis.precedence`)을 읽힌다.
 * - `named-v1` — 이 절이 묻는 물음에 실제로 답하는 판정들을 이름으로 지목한다.
 *
 * ## 왜 셋인가 — `precedence-v1` 이 **겨눈 곳이 틀렸다**
 *
 * 한 번 재보고 알았다. `precedence` 가 서열을 매기는 다섯(억부·조후·종격·격국·통관)은
 * 전부 **「무엇을 쓸 것인가」(용신)에 대한 답**이다. 자기 풀이 쪽 문단 제목이 그것을
 * 그대로 말한다 — 「무엇을 쓰면 좋은지가 갈릴 때」.
 *
 * 그런데 10절이 묻는 것은 **「가까운 사이에서 반복하는 모양·서운할 때의 반응·표현하는
 * 속도」**다. 용신이 답하는 물음이 아니다. 그래서 표본 두 사람 다 `조후` 에서 갈리는데도
 * 두 판의 10절에 **없앨 모순 자체가 없었다** — 도구가 그 자리에 안 닿았다.
 *
 * `named-v1` 은 그 잘못을 고친 쪽이다. 서열이 아니라 **이 절의 물음에 실제로 답하는
 * 판정**(강약·십성 무게·관계·뿌리)을 지목한다. 자기 풀이에서 `expert-v4` 가 한 것과
 * 같은 수이고, 그때는 통했다.
 *
 * `precedence-v1` 을 안 지운다. **재어 보고 안 켠 것을 값으로 남긴다** — 지우면 다음
 * 사람이 같은 시도를 다시 하고, 그때 이 라운드가 알아낸 것이 어디에도 없다.
 */
...
/**
 * **이 절의 물음에 실제로 답하는 판정을 지목한다** — `precedence` 가 못 닿은 자리.
 *
 * 10절은 「반복하는 모양·서운할 때의 반응·표현하는 속도」를 묻는다. 그 답은 용신 계열이
 * 아니라 **강약·십성 무게·관계·뿌리**에 있다. 자기 풀이 3절(`expert-v4`)이 같은 축을
 * 같은 말로 잡는다 — 「제 힘으로 버티는 쪽인가, 둘레에 기대는 쪽인가」.
 *
 * **분류명을 안 부른다.** 지금 기준판은 이름을 안 부르는 판(`plain`)이라, 경로만 대고
 * 뜻은 사람 말로 적는다 — 자기 풀이 쪽이 쓰는 어휘를 그대로 옮겼다.
 *
 * **마지막 줄이 이 판이 재려는 것이다.** 두 판 다 「이 명식이 아니어도 맞는 글」이었고,
 * 그것이 10절의 실제 약점이었다. 금지로 적지 않고 **결론의 모양을 시킨다** — 「일반론을
 * 쓰지 마라」는 무엇을 쓰라는 말이 아니라서 글만 조심스러워진다.
 */
const EACH_PERSON_NAMED = `

**한 사람씩, 그 사람 자료에서 유난히 튀는 것을 짚어 쓴다.** \`charts.a\` 와 \`charts.b\` 를
**각각** 열고 축을 하나 먼저 잡는다 — 제 힘으로 버티는 쪽인가, 둘레에 기대는 쪽인가
(\`analysis.strength\`). 그다음 무엇이 무겁고 어느 쪽이 비어 있는지(\`analysis.tenGods\`),
어느 자리가 부딪히고 있는지(\`relations\`), 무엇이 기댈 데를 못 얻었는지
(\`analysis.rootedness\`)를 본다. **유난히 튀는 것이 곧 이 사람이 걸려 있는 자리다** — 그
자리를 가까운 사이에서 실제로 벌어지는 장면으로 옮겨라. 두 사람에게 같은 장면이 나오면
둘 중 하나는 자료를 안 본 것이다.`;

/** 셋을 표로 든다 — 값이 늘면 컴파일러가 빈 칸을 센다 */
const EACH_PERSON_RULE: Record<EachPersonJudgements, string> = {
  unranked: '',
  'precedence-v1': EACH_PERSON_PRECEDENCE,
  'named-v1': EACH_PERSON_NAMED,
};
...
**둘을 견주지 말고 각각 쓴다** — 견주는 것은 앞 절들이 이미 했다.${
    EACH_PERSON_RULE[assembly.eachPersonJudgements]
  }`,
  ...
  return [
    ...
    /*
      ...
      **그리고 재보니 이 문단은 private 에 붙일 것이 아니었다.** 여기가 서열을 매기는
      ...
      하려면 서열이 아니라 그 물음에 답하는 판정을 지목해야 한다(`EACH_PERSON_NAMED`).
      한 번 재보고 알았고, 그 라운드는 `PAIR_VARIANTS` 가 값으로 든다.
    */
