 ...
 *
 * 개인은 이 사람을 한눈에 보면 어떤 형세인지, 궁합은 둘이 함께 있을 때 어떤 형세인지
 * 바로 알아듣게 한다. **비유의 소재와 문장 꼴은 모델이 고른다.** 생활 장면만 쓰게 하거나
 * 「사람을 규정하지 마라」 같은 우회 규칙을 겹치면 모델은 뜻 대신 「~하는 모습이에요」로
 * 문장을 완충했다. 지킬 것은 소재가 아니라 이 한 문장이 대상을 알아보게 하는가이다.
 *
...
\`metaphor\` 에 **한 문장**을 넣는다. ${
  isSolo(kind)
    ? '**이 사람을 한눈에 보면 어떤 형세인지**'
    : '**이 두 사람이 함께 있을 때 어떤 형세인지**'
} 바로 알아듣게 하는 비유다. 자료에서 가장 두드러진 것을 잡되, **비유의 소재와 문장 꼴은 자유롭게 고른다.** 설명이나 해설을 덧붙이지 말고 비유 한 문장만 쓴다.

이 칸에는 본문의 해요체 규칙을 적용하지 않는다. 존댓말 문장으로 닫아도 되고 짧은 비유형으로 끝내도 된다.
