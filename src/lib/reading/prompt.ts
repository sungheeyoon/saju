 ...
 * ## 경계 둘은 우리가 쥔다
 *
 * **하나 — 오행·십성을 비유로 되살리지 못하게 한다.** 「물과 불처럼」·「나무가 물을
 * 만난 격」은 비유의 탈을 쓴 분류명이다. `plain` 판이 어렵게 걷어낸 것을 뒷문으로
 * 들여오는 자리이고, 경로 유출 검사는 **낱말이 한국어라 못 잡는다.**
 *
 * **둘 — 사람을 규정하지 않는다.** 비유는 점수보다 훨씬 세게 꽂힌다. 「물과 기름」이
 * 어머니와의 궁합에 나가면 그것은 제품 사고다. 관계가 어떻게 될지를 예언하지 말고
 * **지금 두 사람이 놓인 자리**를 그리게 한다 — 본문이 그것을 어떻게 풀어 가는지
 * 이어서 말하므로, 비유는 문을 여는 말이지 판결이 아니다.
 *
 * **금지 낱말 목록을 안 만든다.** 막을 것은 낱말이 아니라 문형이라, 「무엇을 쓰라」로
 * 시킨다 — 생활에서 본 장면. 그 자리에서 오행 비유는 저절로 안 나온다.
 *
 ...
 * 실패다.
 */
const METAPHOR_SECTION = (kind: ReadingKind): string => `## 한마디로 빗대면

\`metaphor\` 에 **한 문장**을 넣는다. ${
  isSolo(kind)
    ? '이 사람이 어떤 사람인지'
    : '이 두 사람이 함께 있을 때 어떤 모양인지'
}를 **생활에서 본 장면**으로 빗댄 말이다. **${READING_POLICY.metaphorLength.target}자 안팎으로 짧게** 쓴다 — 화면에서 한 줄로 선다.

「돌고래가 물 만난 격」처럼 **읽는 순간 그림이 서는 말**로 쓴다. 자료에서 가장 두드러진
것을 골라 빗대되, **오행이나 분류 이름을 그대로 비유로 옮기지 마라** — 「물과 불처럼」·
「나무가 물을 만난 격」은 빗댄 것이 아니라 분류명을 다르게 적은 것이다.

${
  isSolo(kind)
    ? '**사람을 규정하지 마라.** 「~한 사람이다」로 못 박지 말고, 이 사람이 살아가는 모양이 보이게 쓴다.'
    : '**관계가 어떻게 될지 예언하지 마라.** 「오래 못 간다」·「천생연분」처럼 결말을 못 박지 말고, **지금 두 사람이 놓인 자리**를 그린다. 그 자리를 어떻게 살아갈지는 본문이 이어서 말한다.'
}
...
${
  isScored(kind)
    ? `\`score\`에는 ${READING_POLICY.scoreRange.min}~${READING_POLICY.scoreRange.max} 정수를, \`metaphor\`에는 한 문장 비유를, \`markdown\`에는 본문을 넣는다.`
    : '\`score\`는 반드시 \`null\`이고, \`metaphor\`에는 한 문장 비유를, \`markdown\`에는 본문을 넣는다.'
}
