...
export const READING_POLICY = {
  ...
  /**
   * 기준점에서 움직일 수 있는 폭 — **프롬프트가 시키고 검사가 막는 같은 수.**
   *
   * 없으면 기준점 65 에서 25 를 깎아 40 을 만들 수 있고, 그러면 기준점을 준 보람이 없다.
   *
   * ## 15 에서 30 으로 넓혔다 — **잰 값이 아니라 제품 결정이다**
   *
   * 15 는 「기준점이 50~77 에 퍼지니 ±15 면 35~92 가 되어 후보 카드의 밴드를 덮는다」는
   * 계산에서 나온 수였다. 그 계산은 맞지만 **실제로 쓰이는 폭은 그보다 훨씬 좁았다** —
   * 아래 항목별 폭을 다 더해도 올리는 쪽이 +14 라, 총합 상한이 위에서는 한 번도 안
   * 걸렸다. 기준점 65 인 흔한 짝은 아무리 잘 맞아도 79 가 천장이었고, 그래서 **「이 둘은
   * 정말 잘 맞는다」를 점수로 말할 방법이 없었다.**
   *
   * 그래서 **항목별 폭과 함께** 두 배로 올린다. 둘 중 하나만 올리면 아무것도 안 바뀐다 —
   * 묶고 있던 것이 총합이 아니라 항목이었기 때문이다.
   *
   * **이것은 재서 정한 수가 아니다.** 넓히면 눈금이 퍼지는지는 실호출로 확인해야 하고,
   * 아직 안 했다. 확인하기 전까지 이 수는 「그렇게 하기로 정했다」까지만 참이다.
   */
  scoreAdjustment: 30,
