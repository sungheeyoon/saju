 ...
 * AI 가 낸다** — `match-v0` 와 후보 카드의 `discovery-v1` 첫인상 점수는 프롬프트에
 * 들어가지 않는다. 그 지표를 함께 넘긴 판본은
...
export const READING_POLICY = {
   ...
   */
  version: 'reading-prompt-v9',
  /**
   * 엔진과 AI 의 경계 — **첫 기준선.**
   *
   * 근거만 넘기고 최종 점수도 모델이 낸다. `match-v0` 와 후보 카드의 `discovery-v1`
   * 첫인상 점수는 프롬프트에 넣지 않는다 — 넣으면 모델 점수가 앞선 숫자를 거의 따라가고,
   * 그러면 「AI 가 스스로 종합하면 무엇이 나오는가」를 한 번도 재지 못한다.
   */
