
/**
 * **현재 AI 결과의 계약** — kind 와 판본과 상한.
 *
 * 자료를 만드는 일(`index.ts`)·프롬프트를 짓는 일(`prompt.ts`)·검사하는 일(`check.ts`)이
 * 다 이 값을 읽는다. 한 자리에 두는 것은 셋이 서로를 부르지 않게 하기 위해서다 —
 * 서로 부르면 불러오는 차례에 기대게 되고, 그때 상수 하나가 `undefined` 로 선다.
 *
 * 아래는 이 파이프라인 전체에 대한 설명이다.
 *
 * **현재 AI 결과를 만드는 파이프라인의 순수한 절반.**
 *
 * 근거를 자르고 · 프롬프트를 짓고 · 나온 것을 검사하는 데까지가 여기다. 부르는 일과
 * 저장하는 일은 서버가 한다(`app/me/reading`). 갈라 두는 까닭은 이 절반이 **DB 도
 * 네트워크도 없이 그대로 시험되는 자리**이기 때문이다 — 검사 규칙이 실제 호출 없이
 * 돌지 않으면 hard fail 이 한 번도 안 걸린 채 배포된다.
 *
 * ## 세 kind 는 **한 파이프라인**이다
 *
 * `self`(내 명식 하나) · `person`(내가 관리하는 저장된 사람 하나) ·
 * `private`(내가 접근 가능한 두 사람) · `match`(성립한 Match).
 * 갈리는 것은 **근거 범위와 접근 판정 둘뿐**이고 나머지는 같은 길을 지난다. kind 마다
 * 파이프라인을 따로 만들면 출력 검사가 한 갈래에서만 도는 일이 생긴다(`prd-archive`).
 *
 * ## 엔진과 AI 의 경계는 **여기서 정하지 않는다**
 *
 * 9단계는 그 경계를 실험하는 단계다(`prd-archive`·ADR 0003). 첫 기준선은 **근거만 넘기고
 * 점수도 AI 가 낸다**였고, 그 판이 무엇을 내는지는 이제 안다 — 62~68 뿐이다(ADR 0052).
 * 현재 판은 **엔진 밖 정책층이 기준점을 내고 모델이 항목별로 움직인다**(ADR 0060).
 * 무엇을 넘겼는지는 판본 이름이 든다(`promptVersion`).
 */

export const READING_KINDS = ['self', 'person', 'private', 'match'] as const;
export type ReadingKind = (typeof READING_KINDS)[number];

/**
 * 한 사람의 명식으로 나는 kind — **`self` 와 `person` 은 같은 계열이다.**
 *
 * 자료도 프롬프트도 검사도 같다. 갈리는 것은 접근 판정 하나뿐이다 — `self` 는 부른
 * 사람의 selfPerson 을 스스로 찾고, `person` 은 `user_person_access` 에 있는 Person
 * 하나를 받는다.
 *
 * **그렇다고 한 낱말로 합치지 않는다.** 합치면 「누구 것을 모델에 넘겼는가」가 기록에서
 * 사라진다 — 내 명식을 넘긴 것과 남의 명식을 넘긴 것은 동의 범위가 다른 일이다.
 *
 * `kind === 'self'` 라고 적힌 자리를 이 술어로 바꾼 것이 이 갈래를 넣는 일의 절반이었다.
 * 「자기 풀이인가」를 물어야 할 자리와 「한 사람짜리인가」를 물어야 할 자리가 그동안
 * 같은 문장이었기 때문이다.
 */
export const SOLO_KINDS = ['self', 'person'] as const;
type SoloKind = (typeof SOLO_KINDS)[number];
export const isSolo = (kind: ReadingKind): kind is SoloKind =>
  (SOLO_KINDS as readonly string[]).includes(kind);

/** 두 사람 사이를 읽는 kind — 점수가 나는 쪽이다 */
export type PairKind = Exclude<ReadingKind, SoloKind>;

/** 점수를 내는 kind — 한 사람짜리 풀이에는 궁합 점수를 억지로 붙이지 않는다 */
const SCORED_KINDS: readonly ReadingKind[] = ['private', 'match'];
export const isScored = (kind: ReadingKind): boolean => SCORED_KINDS.includes(kind);

export const READING_POLICY = {
  /**
   * 프롬프트 판본 — **바뀌면 이름이 바뀐다.**
   *
   * 저장된 결과가 무엇으로 만들어졌는지 되짚는 유일한 값이다. 사용자에게는 보이지
   * 않는다(`prd-archive`: 내부 엔진·prompt·모델 버전은 노출하지 않는다).
   *
   * `v10` 은 **기준점 + 항목 조정 ±15 + 재량 ±10** 이다(ADR 0060·0065). 재량은 이름을
   * 새로 달지 않고 들어왔고 그것까지를 v10 으로 친다 — 이름이 붙은 2026-09-11 부터 재량이
   * 들어온 09-13 사이의 궁합 풀이는 재량 없이 났으므로, 그 이틀치는 만든 시각으로 가른다.
   *
   * **궁합은 따로 센다**(`pairVersion`). 2026-09-15 에 궁합 두 kind 의 프롬프트(읽는 법 4판 · 자리가 붙은
   * 사실 목록 · 요약은 본문 뒤)를 바꾸고 인연 궁합 입력을 제한형 A 로 바꿨다. 개인 풀이는 그대로다 — 한 이름으로
   * 올리면 안 바뀐 kind 의 새 결과가 바뀐 것처럼 기록된다. 이름은 `promptVersionOf` 로 고른다.
   *
   * `v13`(2026-09-23, G-33): 개인 풀이에도 자리가 붙은 사실 목록이 서고, 모든 kind 의 목록 줄에 번호가
   * 붙으며 맨 아래 자리 색인이 선다. v10 → v13 으로 건너뛰는 것은 v11 · v12 가 궁합이 쓴 이름이라서다 —
   * 같은 이름이 두 프롬프트를 가리키지 않게 두 칸이 같은 수로 오른다.
   *
   * `v14`(2026-09-23, G-55): 공통 규칙 조각이 「명식」·「원국」·「충」 대신 사용자에게 쓸 말(「타고난 구조」·
   * 「사주」·「맞부딪힘」)로 가르치고, 쉬운 말 규칙에 본보기 한 쌍이 선다. 시간 미상 규칙(`ABSORPTION_RULE`)은
   * 궁합 읽는 법에도 실리므로 `pairVersion` 도 함께 오른다.
   *
   * `v15`(2026-09-23, G-56): 개인 풀이 규칙이 「일간」·「대운」·「세운」·「월운」을 스스로 부르지 않고
   * (단정의 예는 「金이 셋이다」, 운 칸은 「십 년 단위의 큰 흐름」·「그해의 흐름」·「그달의 흐름」), 쉬운 말
   * 규칙에 일간 · 대운 본보기 한 쌍이 더 선다. 바뀐 조각은 개인 풀이에만 실리므로 `pairVersion` 은 v14 그대로다.
   */
  version: 'reading-prompt-v15',
  /**
   * 궁합 판본 — `v13`(ADR 0067 · G-33).
   * - v12: 근거 칸의 특정 소재 본보기를 걷고, 오행 보완 줄을 고른 사이의 장면으로 읽게 했다(2026-09-15)
   * - 인연 궁합: 제한형 A 입력 + 자리가 붙은 사실 목록 + 궁합 읽는 법 4판 + 한 줄 요약을 본문 뒤에
   * - 비공개 궁합: 입력은 그대로(두 원국 전체) + 같은 목록·읽는 법 4판(그 자료에 맞춘 안내)·요약 차례
   * - v13: 목록 줄의 번호와 자리 색인(G-33, 2026-09-23). 궁합 몸통은 그대로다
   * - v14: 읽는 법에 실린 시간 미상 규칙이 「명식」 대신 「사주」로 적힌다(G-55, 2026-09-23). 나머지 궁합 지시는 그대로다
   * - v16: 점수 절이 기준점을 「사이에 맞는 정책으로 계산한 베타 기준점」으로 설명하고(연인 · 배우자 40 · 40 · 20,
   *   그 밖 60 · 40), 기준점 블록의 옛 두 축 설명을 걷었다(ADR 0113, 2026-09-25). v15 는 개인 풀이가 쓴 이름이라 건너뛴다
   * - v17: 기준점에 든 사실(필요한 기운 보완 · 함께 없는 오행 · 연인 · 배우자의 일간 · 일지 관계)을 조정에서 다시 세지 않는다 —
   *   조정표의 「용신을 상대가 가졌다」·「둘 다 없는 오행」 두 줄을 걷고, 조정 근거는 년 · 월 · 시가 걸린 관계로만(ADR 0113, 2026-09-25)
   */
  pairVersion: 'reading-prompt-v17',
  /**
   * 엔진과 AI 의 경계 — **기준점 위에서 항목별로 움직인다**(ADR 0060).
   *
   * ## 첫 기준선은 무엇이었고 왜 접었나
   *
   * 근거만 넘기고 최종 점수도 모델이 냈다. 후보 카드의 `discovery-v1` 점수를 일부러 안
   * 넣었는데, 「넣으면 모델 점수가 앞선 숫자를 거의 따라가고 그러면 **AI 가 스스로
   * 종합하면 무엇이 나오는가**를 한 번도 재지 못한다」는 까닭이었다.
   *
   * **그 실험은 돌았고 답이 나왔다.** 실호출 열한 번이 전부 62~68 이고 같은 짝 재호출
   * 흔들림이 ±3 이다(ADR 0052) — 스스로 종합하면 눈금이 안 생긴다. 답이 나온 실험을 계속
   * 돌리는 것은 재는 것이 아니라 같은 값을 다시 내는 것이다.
   *
   * 그래서 기준점을 넣는다. 「앞선 숫자를 따라간다」는 이제 막을 것이 아니라 **노리는
   * 것**이다 — 카드에서 74 를 보고 들어온 사람이 65 를 받으면 돈을 쓰고 점수가 내려간다.
   *
   * 기준점은 후보 카드의 스냅샷이 아니라 **풀이 때 두 명식에서 다시 잰 값**이다
   * (`previewScoreOf`). 넘겨받으면 카드를 안 거치는 `private` 에만 기준점이 없어서 두
   * kind 가 다른 길을 탄다. 눈금은 사이가 고르고(ADR 0113), 무슨 눈금이었는지는 풀이와 함께 저장된다.
   */
  boundary: 'model-adjusts-a-computed-baseline',
  index: 'v2-beta-baseline-in-prompt',
  /**
   * **항목으로 설명되는 폭** — 프롬프트가 시키고 검사가 막는 같은 수.
   *
   * 없으면 기준점 65 에서 25 를 깎아 40 을 만들 수 있고, 그러면 기준점을 준 보람이 없다.
   * 15 인 것은 기준점이 p5~p95 로 50~77 에 퍼져서, ±15 면 후보 카드의
   * 밴드(78·72·66·60·54·46)와 같은 축을 덮기 때문이다 — 둘 다 `discovery-v1` 의 수다. `v2-beta` 의
   * 기준점은 p5~p95 가 연인용 36~74 · 일반 35~84 로 넓어졌고 카드의 칸은 75·66·58·50·43·33 이다
   * (`docs/notes/2026-09-25-compat-formula-comparison.md`). 폭 15 는 그 뒤로 다시 재지 않았다.
   *
   * 여기서 움직인 값은 **자료로 설명된다.** 어느 항목이 걸려서 얼마를 움직였는지 표가
   * 있고, 그 표를 프롬프트가 들고 있다.
   */
  scoreAdjustment: 15,
  /**
   * **항목으로 설명되지 않는 폭** — 마지막에 모델이 제 판단으로 얹는 자리.
   *
   * ## 왜 따로 두나
   *
   * 항목 표가 다 담지 못하는 것이 있다. 십성이 서로를 무엇으로 보는지, 관계가 **어느
   * 자리**에 걸렸는지(일지에 걸린 충과 시지에 걸린 충은 무게가 다르다), 시각을 몰라
   * 못 본 것이 얼마나 되는지. 그것을 항목으로 만들려면 우리가 명리 판단을 표로 지어야
   * 하고, 그건 우리가 할 수 있는 일이 아니다.
   *
   * ## 왜 항목 안에 섞지 않나
   *
   * 섞으면 **어디까지가 자료이고 어디부터가 판단인지** 아무도 모르게 된다. 갈라 두면
   * 나중에 「자료 쪽이 문제인가 판단 쪽이 문제인가」를 물을 수 있다 — 항목 합과 재량을
   * 따로 세어 보면 되기 때문이다.
   *
   * ## 왜 10 인가
   *
   * **잰 값이 아니다.** 항목 합이 올리는 쪽으로 다 더해도 +14 라 기준점 65 인 짝의
   * 천장이 79 였고, 「이 둘은 정말 잘 맞는다」를 말할 자리가 없었다. 10 을 얹으면
   * 89 까지 열린다. 그 정도면 말할 수 있고, 기준점이 힌트로 전락하지도 않는다 —
   * 합쳐서 ±25 는 기준점이 퍼진 폭(27)과 비슷한 크기다.
   *
   * 한동안 이 자리가 없고 항목 폭을 두 배로 올리는 판이 서 있었다. 그러면 **상한이
   * 한쪽에서만 걸리고**(올리는 쪽 합이 +28 로 ±30 에 안 닿았다) 무엇이 자료이고
   * 무엇이 판단인지도 갈리지 않는다.
   */
  scoreDiscretion: 10,
  /** 출력은 한 생성 건에서 점수와 글이 함께 나온다 — 따로 갱신하지 않는다 */
  output: 'score-and-markdown-from-one-generation',
  /** 점수의 범위. 검사가 이 값을 읽는다 */
  scoreRange: { min: 0, max: 100 },
  /** 글의 길이 — 너무 짧으면 근거를 안 읽은 것이고, 너무 길면 읽히지 않는다 */
  markdownLength: { min: 400, max: 12000 },
  /**
   * 한 줄 요약의 길이 — **시키는 값과 막는 값이 다르다.**
   *
   * 프로덕션에서 자기 풀이가 이것 때문에 떨어졌다(`63자 (60 이하)`). 까닭은 길이가
   * 아니라 **프롬프트가 길이를 한 번도 말하지 않은 것**이었다 — 모델이 지킬 방법이 없는
   * 계약을 검사만 들고 있었다. 토큰은 나가고 글은 버려졌다.
   *
   * 그래서 둘로 가른다.
   *
   * - `target` — 프롬프트가 **시키는** 값. 여기서 읽어다 지시문에 꽂으므로 두 벌이 안 된다.
   * - `max` — 검사가 **막는** 값. 「내 취향보다 길다」가 아니라 **「이건 한 문장이
   *   아니다」**를 막는 자리다. 화면이 점수 위에 한 줄로 세우는데 문단이 오면 그 배치가
   *   깨진다 — 막는 값은 그 경계에 둔다.
   *
   * 「재어 보고 안 켠 것을 값으로 남긴다」의 반대 실수를 했다: **재보지도 않은 수를 켰다.**
   */
  metaphorLength: { target: 40, max: 120 },
} as const;

/** 모델이 내야 하는 것 — 화면은 이 둘만 안다 */
export type ReadingOutput = {
  /** 궁합만. 자기 풀이는 `null` */
  score: number | null;
  /**
   * **한 줄 요약** — 점수가 못 하는 일을 이것이 한다.
   *
   * 실호출 산출물을 세어 보니 궁합 점수 열한 번이 전부 62~68 이었다. 0~100 이라고
   * 말하면서 실제로 쓰는 폭이 7 점이고, **같은 짝을 다시 불렀을 때의 흔들림(±3)이 다른
   * 짝과의 차이만큼 크다.** 그 숫자로는 두 관계를 구별할 수 없다.
   *
   * 눈금을 고치는 대신 **숫자에서 의미를 내렸다.** 66 과 68 을 구별하라고 안 시키고,
   * 이 둘이 어떤 사이인지는 직접적인 한 줄 요약이 진다 — 같은 62~68 안에서도 전혀 다른 두
   * 관계를 말할 수 있는 자리다.
   *
   * 자기 풀이에는 점수가 없지만 요약은 있다. 「이 사람의 핵심」이 그 자리다.
   */
  metaphor: string;
  /** 원문 Markdown. 화면은 절 구조를 알지 않는다 */
  markdown: string;
};

/** 그 kind 의 결과에 적을 프롬프트 판본 — **저장하는 자리는 이것만 부른다** */
export const promptVersionOf = (kind: ReadingKind): string =>
  isSolo(kind) ? READING_POLICY.version : READING_POLICY.pairVersion;
