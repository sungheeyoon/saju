...
/**
 * # `src/lib/input` — **입력이 어디서 와서 어떻게 명식이 되는가**
 *
 * 셋이 한 사슬이다. `query` 가 **입력** 한 벌의 모양을 정하고, `edit` 이 저장된
 * 판본 한 줄을 그 모양으로 좁히고, `chart` 가 거기서 **명식**을 낸다. 폼·주소·DB 세
 * 군데서 오는 것이 같은 한 벌로 모이는 자리다.
 *
 * ## `app/` 이 아니라 여기 있는 까닭
 *
 * 셋 다 DB 를 한 번도 안 만지고 JSX 도 없다. `app/` 루트에 있는 동안에는 라우트와
 * 구별되지 않았고 — 그 자리에 서른 개가 섞여 있었다 — 순수한 것을 순수하지 않은 것과
 * 같은 층에 두면 「이건 어디서 도는가」를 파일마다 열어 봐야 안다. **`src/lib` 은 DB 를
 * 안 만진다**는 규율이 이미 있었으므로, 그 규율이 그대로 답이었다.
 *
 * 배럴(`index.ts`)을 안 둔다. 셋이 하는 일이 달라서 한 이름으로 묶으면 「이 안에 무엇이
 * 공개인가」를 다시 알 수 없게 된다(ADR 0047 이 엔진 배럴에서 고친 것이 그것이다).
 * 부르는 쪽이 셋 중 무엇에 기대는지 경로가 말한다.
 *
 * ## `src/lib/saju/input.ts` 와 다르다
 *
 * 저쪽은 **엔진이 무엇을 거부하는가**의 계약이다(지원 연도, 없는 날짜). 여기는
 * **입력이 어디서 오는가**의 길이다. 저쪽은 엔진 안에서만 경로로 불리고 밖에서는 배럴을
 * 지나므로, 두 이름이 한 import 줄에 나란히 설 일은 없다.
 *
 * ---
 *
...
export type Query = {
   ...
   * 그 시절의 변환을 붙들어야 해서 예외이고, 그 대조는 `edit.ts` 가 한다.
   */
  date: string;
  time: string;
  /**
 ...
 *
 * 궁합 계산기 안에 있었다. 같은 링크를 읽는 자리가 둘이 되면서(`/evidence` 가 결과
 * 화면의 주소를 그대로 열었다) 옮겼다 — 접두사를 읽는 쪽이 제 손으로 `'a.'` 를 적으면,
 * 접두사가 바뀌는 날 한쪽만 고쳐지고 그때 두 화면이 같은 주소에서 다른 사람을 읽는다.
 *
 * 읽는 자리는 `/evidence` 가 사라지면서 다시 하나가 됐다(ADR 0047). 그래도 코덱 옆에
 * 둔다 — 둘이 됐던 것은 화면이 늘어서였지 이 값이 계산기의 것이어서가 아니다.
 */
