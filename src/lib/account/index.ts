 ...
 * 한다. 다섯 화면이 저마다 「이용이 정지된 계정입니다」를 적고 있었고, 그래서 상태가 하나
 ...
 *
 * 이름과 문구는 `docs/prd.md` §5.3 의 표다(2026-09-23) — 이용 정지 · 탈퇴 대기. 제목 자리라
...
> = {
  ...
  deletion_requested: {
    title: '탈퇴를 신청한 계정입니다',
    detail:
      '탈퇴 신청이 접수되었습니다. 인연 찾기 노출과 새 요청·풀이 생성은 이미 멈췄고, 저장된 자료는 운영자가 처리할 때까지 그대로 있습니다. 되돌리려면 운영자에게 알려 주세요.',
  },
} as const;
 ...
 *
 * 「이용 정지」와 섞지 않는다(ADR 0048). `app_user` 의 select 정책은 `id = auth.uid()` 하나이고
 * `status` 를 안 본다 — 정지된 계정도 자기 행을 읽는다. 그러니 **못 읽었다는 것은 정지의
 * 증거가 아니다.** 한 화면이 이 둘을 섞어서, 아무 일도 없는 사람에게 「이용이 정지된 계정입니다」를
 * 세우고 있었다.
 *
...
export const DELETION_NOTE =
  '탈퇴를 신청하면 인연 찾기 참여가 즉시 꺼지고, 답을 기다리던 요청이 정리되며, 새 요청과 풀이 생성이 막힙니다. 저장된 자료는 탈퇴 신청 후 3일 이내에 자동으로 지웁니다. 함께 보기로 한 궁합이 있다면 그 관계와 결과는 상대 화면에서도 함께 사라집니다. 보낸 메시지는 상대의 대화방에 남고, 대화방이 닫힌 날부터 90일 뒤에 지워집니다. 신고와 관련된 기록은 탈퇴 처리일부터 6개월 동안 따로 보관한 뒤 지웁니다.';
