 ...
 *
 * PRD 가 첫 배포 전에 요구한 측정이다. 고정 표본에서 `discovery-v0` 로 줄을 세웠을 때
 ...
 * 것이 실제로 지켜지고 있다는 뜻이다(PRD 가 감시하라고 한 바로 그 값이다).
 *
 ...
 * 별도 결정으로 적는다(PRD: 공개 전 반드시 다시 결정할 것).
 */
...
describe('discovery-v0 의 노출 분포 — 재어서 남긴다', () => {
  ...
  it('아무도 목록에서 사라지지 않는다', () => {
    /*
      **정책은 정렬만 한다.** 사주 계산값에 하드 threshold 를 두어 후보를 지우지
      않기로 했으므로(PRD), 표본이 한 화면에 들어가는 크기일 때 한 번도 안 서는
  ...
  it('출생시간 유무에 따른 노출 격차를 값으로 남긴다', () => {
    ...
    /*
      **입력 완성도는 순위에 쓰지 않기로 했다**(PRD·ADR 0003). 그래도 격차는 날 수
