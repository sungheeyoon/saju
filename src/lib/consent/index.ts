...
export const MATCH_DISCLOSURE = {
  shown: [
    ...
    '그 관계에 대한 사주풀이와 점수 — 누가 보든 같은 글이고, 어느 한쪽이 새로 만들면 두 분에게 함께 바뀝니다.',
    '궁합을 이루는 일부 오행 구성.',
  ],
  hidden: [
    '정확한 생년월일시와 출생지.',
...
export const CONSENT_FLOW_STEPS = [
  ...
  {
    title: '둘이 같은 결과를 봅니다',
    body: '수락하면 함께 보는 궁합이 열리고, 두 사람 사이에 걸리는 관계와 사주풀이를 양쪽에서 확인할 수 있습니다. 그때도 정확한 생년월일시와 출생지는 열리지 않습니다.',
  },
] as const;
...
export const REVISION_BOUND_NOTE =
  '이 요청은 지금 두 사람의 출생정보 판본에 매여 있습니다. 어느 한쪽이 생년월일시·출생지·계산 옵션을 고치면 요청은 무효가 됩니다 — 동의한 대상과 실제 계산 대상이 달라지기 때문입니다. 그때는 새 입력으로 다시 보내면 됩니다. 이름과 메모를 고치는 것은 무효로 만들지 않습니다.';
...
export const MATCH_RESULT_INTRO =
  '두 분이 서로 동의해 열린 자리입니다. 여기 서는 것은 두 사람 사이에 걸리는 관계와, 그 관계를 읽은 중립적인 사주풀이와 점수입니다.';
...
export const MATCH_RESULT_ENGINE_NOTE =
  '표와 그 아래 문장은 계산 결과에서 곧바로 조립한 것입니다. 그 아래 사주풀이와 점수는 언어 모델이 따로 써서 저장해 둔 것이고, 두 분이 같은 것을 봅니다.';
...
export const MATCH_RESULT_CLOSED_NOTE =
  '지금은 이 결과를 열 수 없습니다. 함께 보기로 한 두 분의 동의는 그대로 있고, 결과를 계산하는 쪽이 지금 그 판본을 읽지 못하는 것입니다.';
