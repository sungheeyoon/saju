...
export const MATCH_DISCLOSURE = {
  shown: [
    ...
    '그 관계에 대한 AI 해석과 점수 — 누가 보든 같은 글이고, 어느 한쪽이 새로 만들면 두 분에게 함께 바뀝니다.',
    '궁합을 이루는 일부 오행 구성.',
  ],
  hidden: [
    '정확한 생년월일시와 출생지.',
...
export const MATCH_RESULT_INTRO =
  '두 분이 서로 동의해 열린 자리입니다. 여기 서는 것은 두 사람 사이에 걸리는 관계와, 그 관계를 읽은 중립적인 AI 해석과 점수입니다.';
 ...
 *
 * 관계 표와 그 아래 문장은 저장소의 문장 조립기가 근거에서 곧장 만든 것이고, AI 해석은
 * 따로 만들어 저장된 현재 결과다. 둘을 구별해 말하지 않으면 사용자는 화면 전체가 한
 * 곳에서 나온 줄로 읽는다.
 *
 * **점수는 AI 해석과 같은 생성 건에서 나온다.** 예전에는 이 자리에 「AI 가 붙어도 점수를
 * 새로 만들지 않고 `match-v0` 를 설명한다」고 적혀 있었다. 그 결정은 폐기됐다 — 엔진과
 * AI 가 최종 점수를 어떻게 나눠 만들지는 9단계에서 실험으로 정하고, 지금 사용자에게
 * 보이는 점수와 글은 **같은 현재 결과 한 벌**이다.
 */
export const MATCH_RESULT_ENGINE_NOTE =
  '표와 그 아래 문장은 계산 결과에서 곧바로 조립한 것입니다. 그 아래 AI 해석과 점수는 따로 만들어 저장해 둔 것이고, 두 분이 같은 것을 봅니다.';
