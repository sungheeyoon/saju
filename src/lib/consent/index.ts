 ...
 * 적으므로 상대의 글자가 그 자리에 선다 — 같은 이유로 뺄 수 없고(그것을 빼면 관계가
 * 아니라 개수가 된다) 같은 이유로 적는다. **고지가 좁아지는 것은 자리가 채워졌다는
 * 증거다.**
 */
export const MATCH_DISCLOSURE = {
  shown: [
    '두 사람 사이에 걸리는 형충회합과 고정된 `match-v0` 지표.',
    '그 관계에 걸린 글자 — 무엇과 무엇이 걸렸는지 적으므로 상대의 글자도 그 자리에서는 보입니다.',
    '그 관계를 설명하는 중립적인 해석 — 누가 보든 같은 글입니다.',
    '궁합을 이루는 일부 오행 구성.',
  ],
  hidden: [
    '정확한 생년월일시와 출생지.',
    '상대의 여덟 글자 전부와 그 위의 판정 — 십성·신살·원국 안의 형충회합, 운.',
