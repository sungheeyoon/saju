...
export const CONSENT_FLOW_CAVEAT =
  '요청은 보낼 때의 두 출생정보에 매여 있습니다. 답을 기다리는 동안 어느 한쪽이 생년월일시·출생지·계산 옵션을 고치면 그 요청은 무효가 되고, 다시 보내야 합니다.';
...
export const REVISION_BOUND_NOTE =
  '이 요청은 지금 두 분이 저장해 두신 출생정보에 매여 있습니다. 어느 한쪽이 생년월일시·출생지·계산 옵션을 고치면 요청은 무효가 됩니다 — 동의한 대상과 실제 계산 대상이 달라지기 때문입니다. 그때는 새 입력으로 다시 보내면 됩니다. 이름과 메모를 고치는 것은 무효로 만들지 않습니다.';
...
export const MATCH_RESULT_PINNED_NOTE =
  '이 결과는 두 분이 동의하신 그때의 출생정보로 계산했습니다. 그 뒤에 어느 쪽이 입력을 고쳤더라도 이 결과는 움직이지 않습니다 — 동의하신 대상이 그때의 출생정보이기 때문입니다. 새 입력으로 다시 보려면 새 요청이 필요합니다.';
...
export const MATCH_RESULT_CLOSED_NOTE =
  '지금은 이 결과를 열 수 없습니다. 함께 보기로 한 두 분의 동의는 그대로 있고, 결과를 계산하는 쪽이 지금 그때의 출생정보를 읽지 못하는 것입니다.';
