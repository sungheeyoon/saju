import { DISCOVERY_POLICY, previewScoreOf } from '@/src/lib/discovery';
...
describe('궁합 베타 지표', () => {
  ...
  it('사주 사실을 두 영역의 버전된 베타 지표로 바꾼다', () => {
    ...
    /**
     * **뺀 두 축은 정책이 이미 뺀 것들이다**(`DISCOVERY_POLICY.excluded`). 카드만
     * 그것을 안 따르고 네 축으로 서 있었다.
     */
    ...
    expect(preview.index).toBe(
      previewScoreOf(charts.a.analysis.elements, charts.b.analysis.elements),
    );
    /**
