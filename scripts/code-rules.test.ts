 ...
 *
 * 남은 자리는 **뒤에서 받치는 쓰기**다. 그래도 조용하다 — 고칠 때 `error` 를 꺼내 기록(`console.error`)에 남기고 여기서
 * 지운다. 풀이의 여섯(`app/me/reading/collect.ts` · `pipeline.ts`)은 2026-09-26 에 그렇게 고쳤다. **넘김:**
 * `app/me/discovery/participation.ts` 는 매칭 화면 담당이다.
 */
...
describe('탈출구의 지문 (docs/agents/code-rules.md) — 줄어들기만 한다', () => {
  ...
  it('DB 결과를 문장으로 버리는 자리는 목록의 옛 자리뿐이다 (ADR 0078)', () => {
