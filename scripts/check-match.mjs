...
try {
  ...
  // ── 4. 받는 쪽 화면 — **동의 화면이다** ─────────────────────────────────────
  {
    ...
    check('열리지 않는 것에 출생 원문과 전체 명식을 적는다',
      html.includes('정확한 생년월일시와 출생지') && html.includes('전체 명식'));
    check('전체 명식 공유는 따로 동의해야 한다고 적는다', html.includes('따로 동의'));
    ...
    /**
     * **낱말로는 못 잰다** — 「전체 명식 — 여덟 글자, 십성·신살·형충회합」은 *열리지
     * 않는 것*의 목록에 적혀 있어야 하는 말이다. 그래서 글자를 본다: 천간과 지지가
     * 한 자라도 응답에 있으면 명식이 새어 나간 것이다. 오행 다섯 글자는 추천 이유라
     * 여기 없다.
     */
