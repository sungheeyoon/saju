...
try {
  ...
  // ── 저장된 두 사람의 궁합 ──────────────────────────────────────────────────
  {
    ...
    /**
     * **사실은 안 선다.** 「둘의 명식 보기」로 접어 두던 두 명식과 관계표를 통째로
     * 내렸다 — 계산이 맞는지 우리가 보려고 세운 원자료이고, 사람이 보러 온 것은
     * 읽어 주는 글이다. 접이칸 낱말만 재면 펼쳐진 채로 돌아와도 통과하므로 그 안에
     * 있던 것을 짚는다(`check-reading.mjs` 도 같은 자리를 본다).
     */
    check('원국·관계 원자료가 로그인 화면에 없다',
