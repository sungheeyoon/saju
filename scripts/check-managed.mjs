...
const PORT = Number(process.env.CHECK_PORT ?? worktreeStack().checkPort + 0);
...
try {
  ...
  // ── 저장된 두 사람의 궁합 ──────────────────────────────────────────────────
  {
     ...
     */
    /* 이 화면은 그 글을 「궁합풀이」라고 부른다 — 도착지가 짓는 이름이다(ADR 0026·0027) */
    check('비공개 궁합에도 풀이 자리가 선다', body.includes('궁합풀이'));
    /**
     ...
     */
    check('내부 판본 이름이 로그인 화면에 없다', !body.includes('match-v0'));
    check('검증 중이라는 것은 그 자리에서 말한다', body.includes('궁합 베타'));
    /**
