...
describe('층의 방향 (ADR 0085)', () => {
   ...
   * 부르는 화면 전부가 DB 와 서버에 묶인다. `app` 안에서는 제 폴더만 안다 — 오행 색(`element-tone`)과 카드(`CARD`)가
   * `app/` 뿌리에 살아 목록으로 풀어 두었던 것을 2026-09-26 에 이 폴더로 옮겨 목록을 걷었다.
   */
  it('공용 부품(app/ui)은 app 안에서 제 폴더만 안다 — supabase 도 모른다', () => {
    const wrong = EDGES.filter(
      (edge) =>
        under(edge.file, 'app/ui') &&
        ((under(edge.target, 'app') && !under(edge.target, 'app/ui')) || (edge.spec !== null && SUPABASE.test(edge.spec))),
    );
    expect(wrong.map(say)).toEqual([]);
    // 빈 폴더로 통과하지 않는다 — 부품이 lib 과 제 폴더를 실제로 부르고 있다
    expect(EDGES.some((edge) => under(edge.file, 'app/ui') && edge.target === 'app/ui/element-tone')).toBe(true);
  });
