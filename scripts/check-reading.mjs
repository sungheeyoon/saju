...
try {
  ...
  // ── 3. 비공개 궁합 ───────────────────────────────────────────────────────
  {
     ...
     * **명식은 아예 안 선다.**
     *
     * 「둘의 명식 보기」로 접어 두던 자리다. 접은 칸도 결과 화면에 「펼치면 뭔가 더
     * 있다」는 자리를 하나 만들었고, 그 안에 든 것은 우리가 계산을 검산하려고 세운
     * 원자료였다 — 사람이 보러 온 것은 읽어 주는 글이다. 그래서 통째로 내렸다.
     *
     * 접이칸의 낱말만 재면 두 명식이 **펼쳐진 채로** 돌아와도 통과한다. 그 안에 있던
     * 것을 함께 짚는다.
     */
    check('둘의 명식이 서지 않는다', !before.includes('둘의 명식 보기'));
