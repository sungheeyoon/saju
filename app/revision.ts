...
export const REVISION_REPLACED_NOTE =
  '고치면 현재 사주와 궁합은 새 입력으로 계산됩니다. 이전에 본 결과와 다를 수 있습니다.';

/**
 * 이전 입력을 얼마나 두는가 — **무한한 수정 이력을 제공하지 않는다**(ADR 0011).
 *
 * 판본을 남기는 이유는 이력을 보여주기 위해서가 아니라 **이미 동의하고 이미 본 결과의
 * 근거를 조용히 바꾸지 않기 위해서**다. 그래서 아무 기록도 가리키지 않게 된 이전
 * 입력까지 영구 보존하지는 않는다.
 */
export const REVISION_RETENTION_NOTE =
  '진행 중인 요청과 함께 보기로 한 궁합이 사용 중인 입력은 그대로 둡니다. 그 밖의 이전 입력은 최근 것만 잠시 남고 정리됩니다.';
