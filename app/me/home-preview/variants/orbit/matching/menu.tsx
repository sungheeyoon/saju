...
/*
  **홈 시안의 제안 메뉴(`../menu.tsx`) 사본 — 켜진 탭만 바깥에서 받는다.**

  원본은 홈 탭이 늘 켜져 있게 적혀 있다(`tab.href === '/me'`). 원본은 같은 스타일의 다른 화면 에이전트가
  함께 읽으므로 고치지 않고, 여기서 `current` 하나만 더했다. 모양 · 크기 · 아이콘은 한 글자도 안 바꿨다.
*/
...
export type MenuHref = (typeof TABS)[number]['href'];

export function TopBar({ unread, unreadChat, current }: { unread: number; unreadChat: number; current: MenuHref }) {
  return (
...
/** 폰의 하단 탭 — 제자리에 선 모형이다 */
export function BottomBar({ unreadChat, current }: { unreadChat: number; current: MenuHref }) {
  return (
