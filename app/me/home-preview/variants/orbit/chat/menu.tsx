...
/*
  홈 시안 `../menu.tsx` 의 사본이다 — 지금 자리를 `current` 로 받는 것만 다르다. 원본은 홈이 박혀 있고,
  같은 스타일의 다른 화면이 함께 읽으므로 고치지 않았다.
...
export function TopBar({ unread, unreadChat, current }: { unread: number; unreadChat: number; current: string }) {
  return (
...
/** 폰의 하단 탭 — 제자리에 선 모형이다 */
export function BottomBar({ unreadChat, current }: { unreadChat: number; current: string }) {
  return (
