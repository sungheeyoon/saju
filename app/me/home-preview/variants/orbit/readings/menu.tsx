...
/*
  **제안 메뉴 사본 — 홈 시안(`../menu.tsx`)과 같고 켜진 탭만 받는다.**

  홈 시안 파일은 다른 화면 에이전트도 읽고 있어 고치지 않는다. 바뀐 것은 `current` 하나다.
...
export type MenuTab = (typeof TABS)[number]['href'];

export function TopBar({ unread, unreadChat, current }: { unread: number; unreadChat: number; current: MenuTab }) {
  return (
...
/** 폰의 하단 탭 — 제자리에 선 모형이다 */
export function BottomBar({ unreadChat, current }: { unreadChat: number; current: MenuTab }) {
  return (
