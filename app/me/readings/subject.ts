...
/**
 * 풀이 대상 한 사람의 **일간** — 책 표지 · 글 표지의 색이 여기서 난다.
 *
 * 풀이 목록의 행(`my_readings()`)에는 명식이 없다. 표지 색 하나를 위해 DB 문을 넓히지 않고, 사람 목록
 * 화면이 이미 쓰는 두 걸음(`storedInputsOf` → `storedChartOf`)을 그대로 밟는다 — 명식을 세우는 함수가
 * 하나여야 목록의 초록 표지와 사람 화면의 초록 타일이 같은 사람을 가리킨다.
 *
