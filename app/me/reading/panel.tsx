...
export function ReadingPanel({
  ...
  const showMock = () => {
    ...
    setIsMock(true);
    setFailure('실제 AI 연결을 사용할 수 없어, 화면 검토용 예시 해석을 표시하고 있습니다.');
    setPhase('idle');
  };
  ...
  const generate = async () => {
    ...
    try {
      ...
      setFailure('예상하지 못한 오류로 해석을 만들지 못했습니다. 잠시 뒤 다시 시도해 주세요.');
      setPhase('error');
      return;
    }
    ...
    <>
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-xl font-bold tracking-tight">현재 AI 해석</h2>
        </div>
        {reading !== null && (
      ...
      <div className="flex flex-col gap-3 border-t border-border pt-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm font-semibold">{reading === null ? '명식 근거로 해석을 만들어 보세요' : '현재 해석을 새로 만들 수 있어요'}</p>
          ...
          >
            {phase === 'loading' ? '해석을 구성하고 있어요…' : reading === null ? '현재 AI 해석 만들기' : '새 해석 만들기'}
          </button>
        </div>
  ...
  return (
    <div className="grid min-h-56 place-items-center rounded-2xl border border-dashed border-border-strong bg-surface-soft px-5 py-10 text-center">
      <div className="max-w-sm">
        ...
        <h3 className="mt-4 text-base font-bold">아직 만들어진 해석이 없어요</h3>
  ...
  return (
    <div role="status" aria-live="polite" className="rounded-2xl bg-accent-wash p-5 sm:p-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="font-semibold">명식의 흐름을 연결하고 있어요</p>
          <p className="text-xs text-secondary">근거를 확인하고, 단정하지 않는 문장으로 정리합니다.</p>
        </div>
      </div>
