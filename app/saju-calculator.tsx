...
function StarTable({ saju }: { saju: Saju }) {
  ...
  return (
    <section id="stars" className={`${CARD} scroll-mt-20`}>
      ...
      <p className="mt-3 border-t border-border pt-3 text-xs text-muted">
        산출법이 갈리는 신살은 채택한 기준을 밝히고 그 기준으로만 뽑습니다 — 고전
        하나로 통일한 것이 아닙니다. 현침은 甲辛卯午申 중 3자 이상, 천문은 戌亥가 함께
        있어야 성립하고, 천의성은 월지 바로 앞 지지입니다. 귀문관살·원진살은 원국 네
        지지의 <strong className="font-medium">모든 쌍</strong>에서 찾습니다(일지 기준으로
        좁히는 계통은 쓰지 않습니다). 역마·도화·화개는 12신살에서, 귀문관살·원진살은 관계
        표에서 가져온 값이라 그쪽과 언제나 일치합니다 — 두 글자가 서로 어떻게 걸렸는지는
        &lsquo;원국의 관계&rsquo;에 있습니다. 길신·흉신은 전통적 분류일 뿐 좋고 나쁨의
        판정이 아닙니다.
