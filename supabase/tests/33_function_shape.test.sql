...
-- ---------------------------------------------------------------------------
-- 2. 로그인 안 한 사람에게 열린 문은 둘뿐이다
-- ---------------------------------------------------------------------------
--
-- 둘 다 그래야 할 까닭이 있다. 베타 일정은 **로그인 화면 앞에서** 읽어야 하고, 공유 풀이는
-- **링크만 든 사람**이 여는 것이다(ADR 0063). 셋째였던 `beta_is_over()` 는 화면이 안 부르고
-- 다른 definer 안에서만 불려 `20261008120000` 이 닫았다(G-23 ⑪, advisor lint 0028).
--
...
select set_eq(
  ...
  $$values ('current_beta_schedule()'),
           ('shared_reading(p_token text)')$$,
  'anon 이 부를 수 있는 public 함수는 이 둘뿐이다');
