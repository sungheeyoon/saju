...
--
-- `discovery_unavailable` 의 **직전 정의는 `20260825120000_match_request.sql:251`** 에 있고,
-- 그 파일이 같은 자리에서 `revoke execute … from anon, public, authenticated` 도 걸었다(277줄).
-- 이 저장소에 그 함수를 정의한 마이그레이션은 그 하나뿐이다 — 전수로 확인했다.
--
-- **그 파일을 베껴 오지는 않았다.** 아래 본문은 스키마에서 떠 온 지금 정의(`pg_get_functiondef`)
-- 에서 **첫 절만** 덜어낸 것이다. 옛 마이그레이션을 베꼈다가 그 사이의 변경이 통째로
-- 되감긴 적이 있어서다(`20260925180000` 이 같은 자리에서 같은 말을 한다).
--
-- 서명이 그대로라 `create or replace` 로 둔다 — 권한이 유지되고 위의 `revoke` 도 그대로 선다.
--
-- ## 차단과 지나침은 그대로다
--
