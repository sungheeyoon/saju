...
/*
  **기존 행은 전부 「아직 결정 안 함」으로 둔다.**

  지금 이 표에서 끈 사람과 안 켠 사람은 구별되지 않는다 — 끄는 길이 `opted_in_at` 과
  `element_summary` 를 함께 비우고, 아무 결정도 안 한 행(조건만 저장한 행)도 같은 두 칸이
  비어 있다. 그래서 이 백필은 자료로 답할 수 없고, **지금 프로덕션에 참여를 켠 적 있는
  계정이 없다는 사실**로만 정당하다(둘 다 이름조차 없어 참여의 문을 지난 적이 없다).

  이 판단은 지금만 싸다. 다음부터는 끈 사건이 기록되므로 같은 물음이 다시 서지 않는다.
*/
